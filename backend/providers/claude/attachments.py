# providers/claude/attachments.py
"""
Best-effort image/file capture for the Capture Center's media viewer -
straight provider-parameterized copy of providers/chatgpt/attachments.py
(same shared ConversationCaptureAttachment table, same R2 upload
infrastructure - see that module's own docstring for the full reasoning).

Unlike ChatGPT's own input-attachment path (DOM file-input/drop
interception, since ChatGPT's API never hands back a fetchable URL for an
uploaded file), Claude's authoritative snapshot already gives every
attached file a same-origin, cookie-authenticated preview URL
(`/api/{orgId}/files/{fileUuid}/preview`) right on the message object - see
CAPTURE_CONTRACT.md. content-claude-capture.js fetches that URL directly and
posts the bytes here, the same way this file's chatgpt sibling captures
OUTPUT (generated) images via resolveAndUploadImagePart.
"""
import base64
import binascii
import time
...
from providers.claude.constants import PROVIDER
...
def _upload_bytes_to_r2(raw_bytes: bytes, *, file_name: str, mime_type: str) -> tuple[str, str]:
    ...
    key = f"claude-capture/{timestamp_ms}/{uuid4().hex[:8]}_{safe_name}"
