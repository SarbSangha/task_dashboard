# providers/claude/capture.py
"""
Raw capture ingestion. Validates just enough to route/dedupe an event, then
stores it losslessly in ConversationCaptureEvent (provider="claude") - the
same shared table providers/chatgpt/capture.py writes to under
provider="chatgpt". See providers/claude/__init__.py for why there is no
providers/claude/models.py.

No parsing, no business logic. Normalization into ConversationRecord/
ConversationPrompt/ConversationResponse is a separate step (normalization.py)
that reads from this table; it does not happen here.
"""
import json
import logging
import time
...
from providers.claude.constants import ALL_EVENT_TYPES, CAPTURE_SCHEMA_VERSION, PROVIDER, TOOL_SLUGS
from routers.it_tools_router import _resolve_tool_credential

logger = logging.getLogger("claude_capture")

# Process-local ingest outcome counters for a future metrics endpoint -
# see providers/chatgpt/capture.py's identical counters for why this is
# in-memory rather than a DB table.
...
def resolve_claude_tool(db: Session) -> Optional[ITPortalTool]:
    return (
...
def _log(*, event_type: str, status: str, duration_ms: float, payload_size: int, reason: Optional[str] = None) -> None:
    logger.info(
        "claude_capture_event provider=%s event_type=%s status=%s duration_ms=%s payload_size=%s reason=%s",
        PROVIDER,
        event_type,
        status,
        duration_ms,
...
def ingest_capture_event(
    ...
    *,
    ...
    """Idempotent insert: retrying the same (provider, credential, client_event_id) is
    always a no-op "duplicate" response - never a second row, never an error.

    client_event_id is built deterministically by the extension as
    `claude:{conversationUuid}:{messageUuid}` (see content-claude-capture.js) rather
    than a random uuid4 - Claude's own authoritative snapshot re-observes the same
    message across every later GET of the same conversation (page reload, reopening
    an old chat, a second tab), so a stable key lets those re-observations collapse
    into "duplicate" for free instead of relying solely on retry-of-the-same-attempt
