...
stores it losslessly in EpidemicCaptureEvent. No parsing of the assetUrl/
response payload happens here - see normalization.py for that.

Mirrors providers/envato/capture.py exactly, including the deferred-import
workaround for the same import cycle (providers.epidemicsound.capture ->
routers.it_tools_router -> routers/__init__ -> ... ->
providers.epidemicsound.capture).
"""
import json
import logging
import time
...
from providers.epidemicsound.constants import ALL_EVENT_TYPES, CAPTURE_SCHEMA_VERSION, PROVIDER, TOOL_SLUGS
from providers.epidemicsound.models import EpidemicCaptureEvent
from utils.client_gate import validate_client_for_generation
from utils.task_gate import validate_task_for_generation

logger = logging.getLogger("epidemicsound_capture")
...
class CaptureIngestResult:
    ...
    event: Optional[EpidemicCaptureEvent] = None
    reason: Optional[str] = None


def resolve_epidemicsound_tool(db: Session) -> Optional[ITPortalTool]:
    return (
...
def resolve_epidemicsound_actor(
    *,
    request: Request,
    db: Session,
    tool: ITPortalTool,
    ...
    reasoning as resolve_envato_actor's own docstring."""
...
def resolve_epidemicsound_credential(
    db: Session,
    *,
    tool_id: int,
    user_id: int,
...
def _log(*, event_type: str, status: str, duration_ms: float, payload_size: int, reason: Optional[str] = None) -> None:
    logger.info(
        "epidemicsound_capture_event provider=%s event_type=%s status=%s duration_ms=%s payload_size=%s reason=%s",
        PROVIDER,
        event_type,
        status,
        duration_ms,
...
def ingest_capture_event(
    ...
    *,
    ...
    """Idempotent insert - mirrors providers/envato/capture.py's
    ...
    existing_query = db.query(EpidemicCaptureEvent).filter(
        EpidemicCaptureEvent.provider == PROVIDER,
        EpidemicCaptureEvent.client_event_id == normalized_client_event_id,
    )
    existing_query = (
        existing_query.filter(EpidemicCaptureEvent.credential_id == credential_id)
        if credential_id is not None
        else existing_query.filter(EpidemicCaptureEvent.credential_id.is_(None))
    )
    ...
    if task_id is not None:
        ...
        if task:
            ...
            logger.warning(
                "epidemicsound_capture_event dropped invalid task attribution user_id=%s task_id=%s",
                user.id, task_id,
            )
    ...
    if client_id is not None:
        ...
        if client:
            ...
            logger.warning(
                "epidemicsound_capture_event dropped invalid client attribution user_id=%s client_id=%s",
                user.id, client_id,
            )

    event = EpidemicCaptureEvent(
        tool_id=tool.id,
        ...
        provider_download_id=(download_id or None),
