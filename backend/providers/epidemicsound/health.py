# providers/epidemicsound/health.py
"""
Capture health: tracks the latest known state of each extension install's
local Epidemic Sound retry queue. Mirrors providers/envato/health.py
exactly - see that file's docstring for the full reasoning.
"""
...
from providers.epidemicsound.models import EpidemicCaptureHealth
...
) -> EpidemicCaptureHealth:
    if extension_session_id:
        existing = (
            db.query(EpidemicCaptureHealth)
            .filter(
                EpidemicCaptureHealth.provider == PROVIDER,
                EpidemicCaptureHealth.extension_session_id == extension_session_id,
            )
            .first()
        )
    else:
        existing = (
            ...
            .filter(
                EpidemicCaptureHealth.provider == PROVIDER,
                EpidemicCaptureHealth.user_id == user_id,
                EpidemicCaptureHealth.extension_session_id.is_(None),
            )
            .order_by(EpidemicCaptureHealth.reported_at.desc())
            .first()
        )

    record = existing or EpidemicCaptureHealth(provider=PROVIDER, user_id=user_id)
...
def get_capture_health_for_user(db: Session, *, user_id: int) -> list[EpidemicCaptureHealth]:
    return (
        db.query(EpidemicCaptureHealth)
        .filter(EpidemicCaptureHealth.provider == PROVIDER, EpidemicCaptureHealth.user_id == user_id)
        .order_by(EpidemicCaptureHealth.reported_at.desc())
        .all()
    )


def compute_capture_health_status(record: EpidemicCaptureHealth, *, now: Optional[datetime] = None) -> str:
...
def capture_health_to_dict(record: EpidemicCaptureHealth, *, now: Optional[datetime] = None) -> dict:
    data = record.to_dict()
