...
def _normalize_search_query_event(db: Session, event: FreepikCaptureEvent) -> FreepikSearchQuery:
    """A stock-library search on freepik.com or magnific.com - see
    FreepikSearchQuery's own docstring for why this is a separate table, and
    constants.py's EVENT_TYPE_SEARCH_QUERY for why dispatch here trusts
    event.event_type directly rather than sniffing payload shape (WE define
    this payload envelope entirely, unlike the Freepik-API-shaped payloads
    _extract_fields below handles).

    No freshness gate, unlike FreepikGeneration's ownership resolution: that
    gate exists specifically to protect against a THIRD-PARTY listing
    endpoint surfacing old account history under whoever's ticket happens to
    be active right now. A search-query capture has no equivalent risk - it
    is generated by content-freepik.js at the exact moment of the user's own
    action, so event.user_id (already resolved by the same launch-ticket
    system every other capture in this codebase trusts) is always
    trustworthy here. One row per search, never merged/updated - unlike a
    generation, a search has no "processing -> completed" lifecycle to
    reconcile snapshots of."""
    payload = event.payload_json or {}
    row = FreepikSearchQuery(
        provider=PROVIDER,
        ...
        result_count_label=_s(payload.get("resultCountLabel"), 40),
        filters_json=payload.get("filters") if isinstance(payload.get("filters"), dict) else None,
        searched_at=_parse_dt(payload.get("searchedAt")) or event.created_at,
...
def _normalize_download_click_event(db: Session, event: FreepikCaptureEvent) -> FreepikDownload:
    """A download of an EXISTING (not user-generated) stock asset - see
    FreepikDownload's own docstring. Unlike search, this click is gated
    behind the Task/Client picker (Sarbjeet's own call), so
    linked_task_id/linked_client_id are carried in from the capture event
    exactly like FreepikGeneration's own columns of the same name -
    capture.py has already re-validated both against the live task/client
    tables before this ever runs (see ingest_capture_event), so they're
    trusted as-is here, no re-validation needed a second time."""
    payload = event.payload_json or {}
    row = FreepikDownload(
        provider=PROVIDER,
        source_capture_event_id=event.id,
...
def normalize_capture_event(db: Session, event: FreepikCaptureEvent) -> Optional[Union[FreepikGeneration, FreepikSearchQuery, FreepikDownload]]:
    # Search/download route to their own tables entirely - dispatched first,
    # before _extract_fields (which reads Freepik-API generation fields these
    # payloads don't have) ever runs. See both helpers' own docstrings.
    if event.event_type == EVENT_TYPE_SEARCH_QUERY:
        return _normalize_search_query_event(db, event)
