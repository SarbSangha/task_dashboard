...
stores it losslessly in GrammarlyCaptureEvent. No parsing of doc title/author
happens here - see normalization.py for that.

Mirrors providers/splice/capture.py exactly, including the deferred-import
workaround for the same import cycle (providers.grammarly_docs.capture ->
routers.it_tools_router -> routers/__init__ -> ... ->
providers.grammarly_docs.capture), and including the same reuse of
_resolve_usage_event_actor/_resolve_usage_event_credential every other
provider's capture.py trusts for ownership - opening a doc in an
authenticated dashboard-launched tab has no more ambiguity about "who" than
a Kling/Freepik generate click does, so there is no reason to invent a
different resolution path for this provider.
"""
import json
import logging
import time
...
from providers.grammarly_docs.constants import ALL_EVENT_TYPES, CAPTURE_SCHEMA_VERSION, PROVIDER, TOOL_SLUGS
from providers.grammarly_docs.models import GrammarlyCaptureEvent
from utils.client_gate import validate_client_for_generation
from utils.task_gate import validate_task_for_generation

logger = logging.getLogger("grammarly_docs_capture")
...
class CaptureIngestResult:
    ...
    event: Optional[GrammarlyCaptureEvent] = None
    reason: Optional[str] = None


def resolve_grammarly_docs_tool(db: Session) -> Optional[ITPortalTool]:
    return (
...
def resolve_grammarly_docs_actor(
    *,
    request: Request,
    db: Session,
    tool: ITPortalTool,
    ...
    """Thin, provider-named wrapper around _resolve_usage_event_actor - same
    reasoning as resolve_splice_actor's own docstring."""
...
def resolve_grammarly_docs_credential(
    db: Session,
    *,
    tool_id: int,
    user_id: int,
...
def _log(*, event_type: str, status: str, duration_ms: float, payload_size: int, reason: Optional[str] = None) -> None:
    logger.info(
        "grammarly_docs_capture_event provider=%s event_type=%s status=%s duration_ms=%s payload_size=%s reason=%s",
        PROVIDER,
        event_type,
        status,
        duration_ms,
...
def ingest_capture_event(
    ...
    *,
    ...
    """Idempotent insert - mirrors providers/splice/capture.py's
    ingest_capture_event exactly (see its docstring for the SAVEPOINT/commit
    contract)."""
    ...
    if normalized_event_type in (
        "doc_open", "doc_session_end", "doc_content_captured", "page_name_updated",
    ) and not normalized_session_key:
        reason = "session_key is required"
        _log(event_type=normalized_event_type, status="rejected", duration_ms=_elapsed_ms(started), payload_size=payload_size, reason=reason)
        _record_stat("rejected")
        return CaptureIngestResult(status="rejected", reason=reason)

    existing_query = db.query(GrammarlyCaptureEvent).filter(
        GrammarlyCaptureEvent.provider == PROVIDER,
        GrammarlyCaptureEvent.client_event_id == normalized_client_event_id,
    )
    existing_query = (
        existing_query.filter(GrammarlyCaptureEvent.credential_id == credential_id)
        if credential_id is not None
        else existing_query.filter(GrammarlyCaptureEvent.credential_id.is_(None))
    )
    ...
    if task_id is not None:
        ...
        if task:
            ...
            logger.warning(
                "grammarly_docs_capture_event dropped invalid task attribution user_id=%s task_id=%s",
                user.id, task_id,
            )
    ...
    if client_id is not None:
        ...
        if client:
            ...
            logger.warning(
                "grammarly_docs_capture_event dropped invalid client attribution user_id=%s client_id=%s",
                user.id, client_id,
            )

    event = GrammarlyCaptureEvent(
        tool_id=tool.id,
