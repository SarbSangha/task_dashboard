...
"""
Pydantic request/response payloads for the HeyGen provider's API surface.
...
class CaptureEventIn(BaseModel):
    ...
    DOM-scraped "Render Scene"/"Generate" click snapshot, or an intercepted
    HeyGen network response, plus routing/identity fields the extension
    ...
    video_id: Optional[str] = Field(default=None, max_length=160)
    project_id: Optional[str] = Field(default=None, max_length=160)
    # True only when the extension is walking a HeyGen history/project listing
    # as part of a reconciliation/backfill sync (see sync.py) - never true for
    # a row observed as a natural consequence of a live Generate/Render Scene
    # click or its own network responses.
    ...
    # the user select (see content-heygen-task-modal.js). Never trusted as-is
    ...
    # Kling/Freepik's usage-event payloads send, resolved the same way by
...
class CaptureEventsRequest(BaseModel):
    """Body of POST /api/providers/heygen/capture/events - always a batch, so
    a queue flush sends everything it has queued in one round trip."""
...
class SyncCursorIn(BaseModel):
    ...
    credential_id: Optional[int] = None
    last_seen_video_id: Optional[str] = Field(default=None, max_length=160)
