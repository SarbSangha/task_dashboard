...
stores it losslessly in SpliceCaptureEvent. No parsing of the graphql
response/source URL happens here - see normalization.py for that.

Mirrors providers/epidemicsound/capture.py exactly, including the deferred-
import workaround for the same import cycle (providers.splice.capture ->
routers.it_tools_router -> routers/__init__ -> ... -> providers.splice.capture).
"""
import json
import logging
import time
...
from providers.splice.constants import ALL_EVENT_TYPES, CAPTURE_SCHEMA_VERSION, PROVIDER, TOOL_SLUGS
from providers.splice.models import SpliceCaptureEvent
from utils.client_gate import validate_client_for_generation
from utils.task_gate import validate_task_for_generation

logger = logging.getLogger("splice_capture")
...
class CaptureIngestResult:
    ...
    event: Optional[SpliceCaptureEvent] = None
    reason: Optional[str] = None


def resolve_splice_tool(db: Session) -> Optional[ITPortalTool]:
    return (
...
def resolve_splice_actor(
    *,
    ...
    reasoning as resolve_epidemicsound_actor's own docstring."""
...
def _log(*, event_type: str, status: str, duration_ms: float, payload_size: int, reason: Optional[str] = None) -> None:
    logger.info(
        "splice_capture_event provider=%s event_type=%s status=%s duration_ms=%s payload_size=%s reason=%s",
        PROVIDER,
        event_type,
        status,
        duration_ms,
...
def ingest_capture_event(
    ...
    *,
    ...
    """Idempotent insert - mirrors providers/epidemicsound/capture.py's
    ...
    existing_query = db.query(SpliceCaptureEvent).filter(
        SpliceCaptureEvent.provider == PROVIDER,
        SpliceCaptureEvent.client_event_id == normalized_client_event_id,
    )
    existing_query = (
        existing_query.filter(SpliceCaptureEvent.credential_id == credential_id)
        if credential_id is not None
        else existing_query.filter(SpliceCaptureEvent.credential_id.is_(None))
    )
    ...
    if task_id is not None:
        ...
        if task:
            ...
            logger.warning(
                "splice_capture_event dropped invalid task attribution user_id=%s task_id=%s",
                user.id, task_id,
            )
    ...
    if client_id is not None:
        ...
        if client:
            ...
            logger.warning(
                "splice_capture_event dropped invalid client attribution user_id=%s client_id=%s",
                user.id, client_id,
            )

    event = SpliceCaptureEvent(
        tool_id=tool.id,
