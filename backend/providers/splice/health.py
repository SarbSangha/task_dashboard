# providers/splice/health.py
"""
Capture health: tracks the latest known state of each extension install's
local Splice retry queue. Mirrors providers/epidemicsound/health.py exactly -
...
from providers.splice.constants import (
...
from providers.splice.models import SpliceCaptureHealth
...
def record_health_ping(
    ...
    *,
    ...
    if extension_session_id:
        existing = (
            db.query(SpliceCaptureHealth)
            .filter(
                SpliceCaptureHealth.provider == PROVIDER,
                SpliceCaptureHealth.extension_session_id == extension_session_id,
            )
            .first()
        )
    else:
        existing = (
            ...
            .filter(
                SpliceCaptureHealth.provider == PROVIDER,
                SpliceCaptureHealth.user_id == user_id,
                SpliceCaptureHealth.extension_session_id.is_(None),
            )
            .order_by(SpliceCaptureHealth.reported_at.desc())
            .first()
        )

    record = existing or SpliceCaptureHealth(provider=PROVIDER, user_id=user_id)
...
def get_capture_health_for_user(db: Session, *, user_id: int) -> list[SpliceCaptureHealth]:
    return (
        db.query(SpliceCaptureHealth)
        .filter(SpliceCaptureHealth.provider == PROVIDER, SpliceCaptureHealth.user_id == user_id)
        .order_by(SpliceCaptureHealth.reported_at.desc())
        .all()
    )


def compute_capture_health_status(record: SpliceCaptureHealth, *, now: Optional[datetime] = None) -> str:
...
def capture_health_to_dict(record: SpliceCaptureHealth, *, now: Optional[datetime] = None) -> dict:
    data = record.to_dict()
