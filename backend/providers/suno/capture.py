...
stores it losslessly in SunoCaptureEvent. No parsing of the Suno clip JSON
happens here - see normalization.py for that. Mirrors
providers/elevenlabs/capture.py verbatim (see that file's own docstring for
the full reasoning) - identity/ownership resolution reuses the exact same
...
from providers.suno.constants import ALL_EVENT_TYPES, CAPTURE_SCHEMA_VERSION, PROVIDER, TOOL_SLUGS
from providers.suno.models import SunoCaptureEvent
...
logger = logging.getLogger("suno_capture")
...
class CaptureIngestResult:
    ...
    event: Optional[SunoCaptureEvent] = None
    reason: Optional[str] = None


def resolve_suno_tool(db: Session) -> Optional[ITPortalTool]:
    return (
...
def resolve_suno_actor(
    *,
    ...
    so every provider-specific caller goes through one place if Suno ever
...
def _log(*, event_type: str, status: str, duration_ms: float, payload_size: int, reason: Optional[str] = None) -> None:
    logger.info(
        "suno_capture_event provider=%s event_type=%s status=%s duration_ms=%s payload_size=%s reason=%s",
        PROVIDER,
        event_type,
        status,
        duration_ms,
...
def ingest_capture_event(
    ...
    *,
    ...
    providers/elevenlabs/capture.py::ingest_capture_event's identical
    docstring for the full reasoning this mirrors.
    """
    ...
    existing_query = db.query(SunoCaptureEvent).filter(
        SunoCaptureEvent.provider == PROVIDER,
        SunoCaptureEvent.client_event_id == normalized_client_event_id,
    )
    existing_query = (
        existing_query.filter(SunoCaptureEvent.credential_id == credential_id)
        if credential_id is not None
        else existing_query.filter(SunoCaptureEvent.credential_id.is_(None))
    )
    ...
    if task_id is not None:
        ...
        if task:
            ...
            logger.warning(
                "suno_capture_event dropped invalid task attribution user_id=%s task_id=%s",
                user.id, task_id,
            )
    ...
    if client_id is not None:
        ...
        if client:
            ...
            logger.warning(
                "suno_capture_event dropped invalid client attribution user_id=%s client_id=%s",
                user.id, client_id,
            )

    event = SunoCaptureEvent(
        tool_id=tool.id,
