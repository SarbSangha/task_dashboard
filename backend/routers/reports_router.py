...
def cost_breakdown(
    ...
    # generations (a plain count, always present) is additive alongside
    # credits/cost - not every integrated provider has a credits model yet
    # (Suno's SunoGeneration.credits_used is permanently null - see that
    # provider's normalization.py docstring; ElevenLabs/Flow/Envato are the
    # same today). Those rows still land here at credits=0/cost=0 - correct,
    # not a bug - but a credits-only view (like the "Credit share by tool"
    # pie chart this feeds) has no way to represent a real, non-zero amount
    # of activity for them: a 0-credit slice of a cost pie chart is
    # indistinguishable from "nothing happened". generations lets the
    # frontend show that activity by count instead, for exactly the
    # providers a cost-only chart would otherwise make invisible.
    by_provider = [
        {"provider": p or "unknown", "credits": float(cr), "cost": round(float(cost), 2), "generations": int(g)}
        for p, cr, cost, g in provider_rows
    ]
