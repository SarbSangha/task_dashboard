...
This Chrome extension is the company auto-fill layer for complex login pages such as ChatGPT/OpenAI, Envato, Freepik, Grammarly, Higgsfield, HeyGen, Kling AI, and custom tool scaffolds like Flow.

## What It Does

- Runs on `chatgpt.com`, `chat.openai.com`, `auth.openai.com`, Google/Microsoft login pages used by ChatGPT, Envato domains such as `elements.envato.com`, `freepik.com`, Grammarly domains such as `grammarly.com`, Higgsfield domains such as `higgsfield.ai`, HeyGen domains such as `heygen.com` and `auth.heygen.com`, and Kling AI domains such as `kling.ai`.
...
- Tool slug: `chatgpt`, `envato`, `freepik`, `grammarly`, `higgsfield`, `heygen`, `kling-ai`, or `flow` if the extension is using slug matching.
...
6. Extension fills email/password, or follows the remembered Google/Microsoft/SSO route when ChatGPT sends that account through another login process.
