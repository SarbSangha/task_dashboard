// content-elevenlabs-task-modal.js — Task Selection Modal.
//
// Direct copy of content-flow-task-modal.js (this file's template), renamed
// Flow->Elevenlabs throughout. See that file's own header comment for the
// full design rationale (Shadow DOM isolation, two independent pickers,
// Task optional / Client required) - reproduced here since this file is the
// one actually loaded for ElevenLabs.
//
// Renders inside a Shadow DOM root - a deliberate departure from plain <div>
// status badges elsewhere in this extension: this is a real interactive form
// blocking a real action, so isolation from the host page's CSS matters.
//
// Shows two INDEPENDENT pickers - Task (content-elevenlabs-task-api.js's
// fetchMyActiveElevenlabsTasks) and Client (fetchActiveElevenlabsClients, an
...
//
// Exposes exactly one function, openElevenlabsTaskSelectionModal(), returning
// a Promise that resolves to { taskId, taskName, clientId, clientName }
// (either pair may be null) on Continue, or null on Cancel/ESC/nothing
// available. content-elevenlabs-capture.js's click interceptor is the only
// caller.

const ELEVENLABS_TASK_MODAL_HOST_ID = 'rmw-elevenlabs-task-gate-host';
const ELEVENLABS_PICKER_SEARCH_THRESHOLD = 10;

let elevenlabsTaskModalOpenPromise = null;

function openElevenlabsTaskSelectionModal() {
  // Guards "double-click Generate while the modal is already open" and
  // "popup opened twice" - both collapse to the one in-flight picker.
  if (elevenlabsTaskModalOpenPromise) return elevenlabsTaskModalOpenPromise;

  elevenlabsTaskModalOpenPromise = new Promise((resolve) => {
    mountElevenlabsTaskModal(resolve);
  }).finally(() => {
    elevenlabsTaskModalOpenPromise = null;
  });

  return elevenlabsTaskModalOpenPromise;
}

function elevenlabsTaskModalStyles() {
  return `
    :host { all: initial; }
    .rmw-overlay {
      position: fixed;
...
function elevenlabsStatusLabel(status) {
  const map = {
...
function escapeElevenlabsTaskHtml(value) {
...
function createElevenlabsPickerSection(container, { key, title, radioGroupName, fetchItems, metaFor, onSelectionChange }) {
  const section = document.createElement('div');
  section.className = 'rmw-section';
  section.innerHTML = `
    <p class="rmw-section-title">${escapeElevenlabsTaskHtml(title)}</p>
    <input class="rmw-search" type="text" placeholder="Search…" style="display:none" aria-label="Search ${escapeElevenlabsTaskHtml(title)}" />
  ...
  function renderOptions(filterText) {
    ...
    visible.forEach((item) => {
      ...
      meta.innerHTML = parts.map((part) => `<span class="rmw-badge">${escapeElevenlabsTaskHtml(part)}</span>`).join('');
  ...
  async function load() {
    ...
    searchEl.style.display = state.items.length > ELEVENLABS_PICKER_SEARCH_THRESHOLD ? 'block' : 'none';
...
function mountElevenlabsTaskModal(resolve) {
  document.getElementById(ELEVENLABS_TASK_MODAL_HOST_ID)?.remove();

  const host = document.createElement('div');
  host.id = ELEVENLABS_TASK_MODAL_HOST_ID;
  const shadow = host.attachShadow({ mode: 'open' });

  const style = document.createElement('style');
  style.textContent = elevenlabsTaskModalStyles();
  ...
  const taskSection = createElevenlabsPickerSection(bodyEl, {
    key: 'task',
    title: 'Select Task',
    radioGroupName: 'rmw-elevenlabs-task-choice',
    onSelectionChange: updateContinueState,
    metaFor: (task) => {
      const parts = [elevenlabsStatusLabel(task.status)];
    ...
    fetchItems: async () => {
      const result = await fetchMyActiveElevenlabsTasks();
      if (!result?.ok) return { ok: false, error: result?.error || 'Unable to load your tasks.' };
      return {
        ok: true,
        items: result.tasks,
        emptyMessage: 'No active tasks found. Please contact your manager or create a task before generating.',
      };
    },
  });

  const clientSection = createElevenlabsPickerSection(bodyEl, {
    key: 'client',
    title: 'Select Client',
    radioGroupName: 'rmw-elevenlabs-client-choice',
    onSelectionChange: updateContinueState,
    metaFor: () => [],
    fetchItems: async () => {
      const result = await fetchActiveElevenlabsClients();
  ...
  // Reported bug on the Freepik/Magnific version of this exact modal:
  // picking an option would leak the click through to the host page's own
  // "click outside closes me" listener. Stop every interaction here from
  // propagating past this dialog's own overlay - this modal is a
  // full-screen overlay while open, so the underlying page has no
  // legitimate reason to see any of these events at all. See
  // content-flow-task-modal.js's identical fix for the full incident
  // writeup.
