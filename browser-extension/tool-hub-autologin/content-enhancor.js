const TOOL_SLUG = 'enhancor';
const PREPARED_LAUNCH_KEY = 'rmw_enhancor_prepared_launch';
...
function ensureStatusBadge() {
  ...
  badge.textContent = `Enhancor auto-login\n${STATE.status}`;
...
function setStatus(message) {
  ...
  ensureStatusBadge().textContent = `Enhancor auto-login\n${message}`;
  console.debug('[RMW Enhancor Auto Login]', message);
}
...
function complete(message = 'Enhancor login complete') {
  STATE.settled = true;
  if (STATE.scheduledTimer) {
    window.clearTimeout(STATE.scheduledTimer);
    STATE.scheduledTimer = null;
...
function findAuthOpenAction() {
  return collectBroadActionCandidates().find((element) => {
    ...
    return (text === 'sign in' || text.includes('sign in') || text.includes('log in'))
      || href.includes('/auth');
  }) || null;
}

function isAuthenticatedEnhancorPage() {
  try {
    const url = new URL(window.location.href);
    const host = normalizeText(url.hostname);
    const path = normalizeText(url.pathname);
    const onEnhancorAppHost = host.includes('app.enhancor.ai') || host.includes('enhancor.ai');
    if (!onEnhancorAppHost) return false;
...
function attemptFlow() {
  ...
  if (isAuthenticatedEnhancorPage()) {
    complete();
    return;
  }
  ...
  if (isGoogleCredential()) {
    ...
    if (attemptOpenEnhancorAuth()) return;
    setStatus('Waiting for Enhancor Google sign-in option');
    return;
  }

  if (!emailInput && !passwordInput) {
    if (attemptOpenEnhancorAuth()) return;
    setStatus('Waiting for Enhancor email sign-in form');
    return;
  }
