...
// - unconfirmed DOM structure, see this file's own top comment. Read BEFORE
// the click is re-dispatched, since the click itself may navigate away or
// alter the DOM.
function collectEnvatoElementsDownloadAssetInfo(button) {
  ...
  const rawTitle = img?.getAttribute('alt') || container?.getAttribute?.('aria-label') || container?.getAttribute?.('title') || document.title || null;
...
document.addEventListener('click', (event) => {
  try {
    const target = findEnvatoElementsDownloadActionTarget(event.target);
    if (!target) return;
