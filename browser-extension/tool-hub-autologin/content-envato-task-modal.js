// content-envato-task-modal.js — Task Selection Modal.
//
...
// content-envato-capture.js's click interceptor is the only caller.

const ENVATO_TASK_MODAL_HOST_ID = 'rmw-envato-task-gate-host';
const ENVATO_PICKER_SEARCH_THRESHOLD = 10;

let envatoTaskModalOpenPromise = null;

function openEnvatoTaskSelectionModal() {
  if (envatoTaskModalOpenPromise) return envatoTaskModalOpenPromise;

  envatoTaskModalOpenPromise = new Promise((resolve) => {
    mountEnvatoTaskModal(resolve);
  }).finally(() => {
    envatoTaskModalOpenPromise = null;
  });

  return envatoTaskModalOpenPromise;
}

function envatoTaskModalStyles() {
  return `
    :host { all: initial; }
    .rmw-overlay {
      position: fixed;
...
function envatoStatusLabel(status) {
  const map = {
...
function escapeEnvatoTaskHtml(value) {
...
// duplicated between them. Mirrors content-freepik-task-modal.js's
// createFreepikPickerSection exactly.
function createEnvatoPickerSection(container, { key, title, radioGroupName, fetchItems, metaFor, onSelectionChange }) {
  const section = document.createElement('div');
  section.className = 'rmw-section';
  section.innerHTML = `
    <p class="rmw-section-title">${escapeEnvatoTaskHtml(title)}</p>
    <input class="rmw-search" type="text" placeholder="Search…" style="display:none" aria-label="Search ${escapeEnvatoTaskHtml(title)}" />
  ...
  function renderOptions(filterText) {
    ...
    visible.forEach((item) => {
      ...
      meta.innerHTML = parts.map((part) => `<span class="rmw-badge">${escapeEnvatoTaskHtml(part)}</span>`).join('');
  ...
  async function load() {
    ...
    searchEl.style.display = state.items.length > ENVATO_PICKER_SEARCH_THRESHOLD ? 'block' : 'none';
...
function mountEnvatoTaskModal(resolve) {
  document.getElementById(ENVATO_TASK_MODAL_HOST_ID)?.remove();

  const host = document.createElement('div');
  host.id = ENVATO_TASK_MODAL_HOST_ID;
  const shadow = host.attachShadow({ mode: 'open' });

  const style = document.createElement('style');
  style.textContent = envatoTaskModalStyles();
  ...
  const taskSection = createEnvatoPickerSection(bodyEl, {
    key: 'task',
    title: 'Select Task',
    radioGroupName: 'rmw-envato-task-choice',
    onSelectionChange: updateContinueState,
    metaFor: (task) => {
      const parts = [envatoStatusLabel(task.status)];
    ...
    fetchItems: async () => {
      const result = await fetchMyActiveEnvatoTasks();
  ...
  const clientSection = createEnvatoPickerSection(bodyEl, {
    key: 'client',
    title: 'Select Client',
    radioGroupName: 'rmw-envato-client-choice',
    onSelectionChange: updateContinueState,
    metaFor: () => [],
    fetchItems: async () => {
      const result = await fetchActiveEnvatoClients();
