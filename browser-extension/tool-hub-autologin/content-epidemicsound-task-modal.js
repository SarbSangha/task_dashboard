// content-epidemicsound-task-modal.js — Task Selection Modal.
//
// Renders inside a Shadow DOM root for CSS isolation from Epidemic Sound's
// own page styles - mirrors content-envato-task-modal.js exactly (see its
// own header comment, itself citing content-freepik-task-modal.js, for the
// full reasoning behind the Shadow DOM choice and the "Client required, Task
// optional" validation rule).
//
// Exposes exactly one function, openEpidemicTaskSelectionModal(), returning
...
// available. content-epidemicsound-capture.js's click interceptor is the
// only caller.
//
// Own host id (rmw-epidemicsound-task-gate-host) and function/const names
// namespaced with "Epidemic" - this file shares its isolated-world global
// scope with content-epidemicsound.js (the login-automation script, its own
// separate manifest.json entry but matching the same host), which declares
// generic top-level names of its own. See content-epidemicsound-capture.js's
// header comment for the fuller version of this reasoning.

const EPIDEMIC_TASK_MODAL_HOST_ID = 'rmw-epidemicsound-task-gate-host';
const EPIDEMIC_PICKER_SEARCH_THRESHOLD = 10;

let epidemicTaskModalOpenPromise = null;

function openEpidemicTaskSelectionModal() {
  if (epidemicTaskModalOpenPromise) return epidemicTaskModalOpenPromise;

  epidemicTaskModalOpenPromise = new Promise((resolve) => {
    mountEpidemicTaskModal(resolve);
  }).finally(() => {
    epidemicTaskModalOpenPromise = null;
  });

  return epidemicTaskModalOpenPromise;
}

function epidemicTaskModalStyles() {
  return `
    :host { all: initial; }
    .rmw-overlay {
      position: fixed;
...
function epidemicStatusLabel(status) {
  const map = {
...
function escapeEpidemicTaskHtml(value) {
...
// duplicated between them. Mirrors content-envato-task-modal.js's
// createEnvatoPickerSection exactly.
function createEpidemicPickerSection(container, { key, title, radioGroupName, fetchItems, metaFor, onSelectionChange }) {
  const section = document.createElement('div');
  section.className = 'rmw-section';
  section.innerHTML = `
    <p class="rmw-section-title">${escapeEpidemicTaskHtml(title)}</p>
    <input class="rmw-search" type="text" placeholder="Search…" style="display:none" aria-label="Search ${escapeEpidemicTaskHtml(title)}" />
  ...
  function renderOptions(filterText) {
    ...
    visible.forEach((item) => {
      ...
      meta.innerHTML = parts.map((part) => `<span class="rmw-badge">${escapeEpidemicTaskHtml(part)}</span>`).join('');
  ...
  async function load() {
    ...
    searchEl.style.display = state.items.length > EPIDEMIC_PICKER_SEARCH_THRESHOLD ? 'block' : 'none';
...
function mountEpidemicTaskModal(resolve) {
  document.getElementById(EPIDEMIC_TASK_MODAL_HOST_ID)?.remove();

  const host = document.createElement('div');
  host.id = EPIDEMIC_TASK_MODAL_HOST_ID;
  ...
  const taskSection = createEpidemicPickerSection(bodyEl, {
    key: 'task',
    title: 'Select Task',
    radioGroupName: 'rmw-epidemic-task-choice',
    onSelectionChange: updateContinueState,
    metaFor: (task) => {
      const parts = [epidemicStatusLabel(task.status)];
    ...
    fetchItems: async () => {
      const result = await fetchMyActiveEpidemicTasks();
  ...
  const clientSection = createEpidemicPickerSection(bodyEl, {
    key: 'client',
    title: 'Select Client',
    radioGroupName: 'rmw-epidemic-client-choice',
    onSelectionChange: updateContinueState,
    metaFor: () => [],
    fetchItems: async () => {
      const result = await fetchActiveEpidemicClients();
