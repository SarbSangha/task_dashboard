const TOOL_SLUG = 'epidemic-sound';
const LOGIN_URL = 'https://www.epidemicsound.com/';
...
const STATE = {
  ...
  status: 'Waiting for Epidemic Sound login form',
};
...
// Epidemic Sound is a Next.js SSR app: acting before React finishes hydrating trips a
...
function ensureStatusBadge() {
  const existing = document.getElementById('rmw-epidemic-autologin-status');
  if (existing) return existing;

  const badge = document.createElement('div');
  badge.id = 'rmw-epidemic-autologin-status';
  ...
  badge.textContent = `Epidemic Sound auto-login ${SCRIPT_VERSION}\n${STATE.status || 'Starting auto-login'}`;
...
function setStatus(message) {
  ...
  console.debug('[RMW Epidemic Sound Auto Login]', message);
}

function debugLog(label, data = {}) {
  console.log(`[RMW Epidemic Sound Auto Login] ${label}`, data);
}

function exposeDebugState() {
  try {
    window.__RMW_STATE = STATE;
    window.__RMW_EPIDEMIC_DEBUG = {
      state: STATE,
      attemptFill,
      attemptOpenEpidemicLogin,
...
function diagnoseActionStep({ label, textValues, finder }) {
  ...
  console.log(`[RMW Epidemic Sound Auto Login] ${label} diagnostic result`, result);
  return result;
}
...
function clickAction(element, label, delays = [0, 250, 900]) {
  if (!element) return false;
  console.log('[RMW Epidemic Sound Auto Login] clicking action', label, describeElement(element));
  delays.forEach((delay) => {
    window.setTimeout(() => {
      if (!isVisible(element) || isDisabled(element)) return;
      console.log('[RMW Epidemic Sound Auto Login] click attempt', label, delay);
      safeClick(element);
...
// Fresh (incognito) sessions always show Epidemic Sound's cookie-consent banner, whose
...
function maybeDismissCookieConsent() {
  ...
  if (consentAction) {
    ...
    debugLog('Dismissing Epidemic Sound cookie consent', describeElement(consentAction));
...
// never leaves login.epidemicsound.com. Resolve the real href and navigate to it.
...
function findGoogleBrokerHref() {
  ...
  for (const anchor of anchors) {
    ...
    const looksLikeGoogleBroker = lowered.includes('/broker/google/')
      || (lowered.includes('broker') && lowered.includes('google'))
      || lowered.includes('kc_idp_hint=google')
      || (lowered.includes('idp') && lowered.includes('google'));
    if (looksLikeGoogleBroker) {
      try {
...
function onEpidemicHost() {
  return window.location.hostname === 'epidemicsound.com'
    || window.location.hostname.endsWith('.epidemicsound.com');
}

// Epidemic Sound uses a dedicated Keycloak login page rather than an on-site
// modal (login.epidemicsound.com), similar to HeyGen's separate auth host.
function onAuthHost() {
  return window.location.hostname === 'login.epidemicsound.com';
}

function isLandingPage() {
  const host = window.location.hostname;
  const path = `${window.location.pathname || '/'}`;
  return (host === 'epidemicsound.com' || host === 'www.epidemicsound.com') && (path === '/' || path === '');
}
...
function isLoginPage() {
  return onAuthHost()
    ...
    || (onEpidemicHost() && Boolean(findHomeSignInAction()));
}

function looksLikeAuthenticatedWorkspace() {
  if (!onEpidemicHost()) return false;
  ...
  const workspaceWords = ['music', 'sound effects', 'my sounds', 'projects', 'browse', 'discover', 'playlists', 'download', 'favorites', 'collections'];
...
function schedulePasswordSwitchAfterEmailChoice(attempt = 1) {
  ...
  STATE.passwordSwitchTimer = window.setTimeout(() => {
    ...
    if (passwordOption || emailInput) {
      setStatus(`Opening Epidemic Sound password sign-in (attempt ${attempt})`);
    ...
    if (attempt < 8) {
      ...
      setStatus('Epidemic Sound password option not found after email choice');
      forceScheduleAttempt(200);
    }
  }, attempt === 1 ? 500 : 750);
}

function attemptOpenEpidemicLogin() {
  debugLog('attemptOpenEpidemicLogin CALLED', {
    canActNow: canActNow(),
  ...
  const emailInput = findEmailInput();
  const passwordInput = findPasswordInput();
  debugLog('attemptOpenEpidemicLogin fields', {
    emailInput: describeElement(emailInput),
    passwordInput: describeElement(passwordInput),
  });
  if (emailInput || passwordInput) {
    debugLog('attemptOpenEpidemicLogin EXIT existing fields');
    return false;
  }
  ...
  if (shouldUseGoogleProvider()) {
    ...
    if (googleOption) {
      if (canActNow()) {
        ...
        setStatus('Opening Epidemic Sound Google sign-in');
  ...
  if (emailOption) {
    if (canActNow()) {
      markActionTaken();
      STATE.loginOpenAttempts += 1;
      setStatus('Opening Epidemic Sound email sign-in');
  ...
  if (homeSignIn) {
    if (canActNow()) {
      markActionTaken();
      STATE.loginOpenAttempts += 1;
      setStatus('Opening Epidemic Sound sign-in');
  ...
  if (!onEpidemicHost() && canActNow()) {
    markActionTaken();
    STATE.loginOpenAttempts += 1;
    setStatus('Redirecting to Epidemic Sound sign-in');
    window.location.replace(LOGIN_URL);
    return true;
  }
...
function attemptProviderChoice(source = 'provider-choice') {
  ...
  setStatus(`Inspecting Epidemic Sound ${route} sign-in (${source})\n${stateSnapshotText()}`);
  ...
  if (route === 'google') {
    setStatus(`Choosing Epidemic Sound Google sign-in (${source})`);
    ...
    if (findHomeSignInAction() && attemptOpenEpidemicLogin()) {
      return true;
    }
    setStatus('Epidemic Sound Google sign-in option not found');
    return false;
  }

  if ((emailInput || passwordInput) && !emailOption && !googleOption) {
    setStatus(`Epidemic Sound form already open (${source})\n${stateSnapshotText()}`);
    return false;
  }

  setStatus(`Choosing Epidemic Sound email sign-in (${source})`);
    if (emailOption) {
      clickAction(emailOption, 'Use email');
  ...
  setStatus('Epidemic Sound email sign-in option not found');
  return false;
}
...
function attemptSwitchToPassword(emailInput) {
  ...
  if (!usePasswordAction) {
    if (canActNow() && clickVisibleText(['use password', 'use password instead', 'sign in with password', 'log in with password'], 'Use password')) {
      markActionTaken();
      setStatus('Switching Epidemic Sound to password login');
  ...
  if (!STATE.credential?.loginIdentifier) {
    requestCredential();
    setStatus('Waiting for Epidemic Sound email credential');
    return true;
  }
...
function stopAutomation(message, hideBadgeAfterMs = 2500) {
  ...
  if (hideBadgeAfterMs > 0) {
    window.setTimeout(() => {
      const badge = document.getElementById('rmw-epidemic-autologin-status');
      if (badge) {
        badge.remove();
      }
    }, hideBadgeAfterMs);
...
function attemptFill() {
  ...
  // On Epidemic Sound's Keycloak login page the "Continue with Google" button is
  ...
  if (onAuthHost()) {
    ...
    if (!isEmailCredential) {
      // Navigate straight to the Google broker anchor WITHOUT fetching the
      // credential here — fetching on the login host churns and can loop. The
      // credential is applied on accounts.google.com by content-google.js.
      const brokerHref = findGoogleBrokerHref();
      debugLog('auth-host broker check', { brokerHref, credentialMethod });
      if (brokerHref) {
        setStatus('Opening Google sign-in');
        window.location.assign(brokerHref);
        STATE.settled = true;
  ...
  if (shouldUseGoogleProvider()) {
    if (attemptProviderChoice('attempt-fill') || attemptOpenEpidemicLogin()) {
      return;
    }
    setStatus('Waiting for Epidemic Sound Google sign-in option');
    return;
  }
  ...
  if (emailInput && !passwordInput) {
    requestPasswordSavingSuppression();
    setStatus('Waiting for Epidemic Sound password login');
    return;
  }

  if (!emailInput && !passwordInput) {
    attemptOpenEpidemicLogin();
    setStatus('Waiting for Epidemic Sound login field');
    return;
  }
...
function scheduleAttempt(delay = 0) {
  if (STATE.settled) {
    ...
    setStatus(`Epidemic Sound scheduler skipped: settled\n${stateSnapshotText()}`);
    return;
  }
  if (STATE.scheduledTimer) {
...
function forceScheduleAttempt(delay = 0) {
  if (STATE.settled) {
    ...
    setStatus(`Epidemic Sound force scheduler skipped: settled\n${stateSnapshotText()}`);
    return;
  }
  if (STATE.scheduledTimer) {
    window.clearTimeout(STATE.scheduledTimer);
