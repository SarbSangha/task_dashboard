// content-flow-task-modal.js — Task Selection Modal.
//
// Direct copy of content-freepik-task-modal.js (this file's template),
// renamed Freepik->Flow throughout. See that file's own header comment for
// the full design rationale (Shadow DOM isolation, two independent pickers,
// Task optional / Client required) - reproduced here since this file is the
// one actually loaded for Flow.
//
...
//
// Shows two INDEPENDENT pickers - Task (content-flow-task-api.js's
// fetchMyActiveFlowTasks) and Client (fetchActiveFlowClients, an
...
//
// Exposes exactly one function, openFlowTaskSelectionModal(), returning a
// Promise that resolves to { taskId, taskName, clientId, clientName } (either
// pair may be null) on Continue, or null on Cancel/ESC/nothing available.
// content-flow.js's click interceptor is the only caller.

const FLOW_TASK_MODAL_HOST_ID = 'rmw-flow-task-gate-host';
const FLOW_PICKER_SEARCH_THRESHOLD = 10;

let flowTaskModalOpenPromise = null;

function openFlowTaskSelectionModal() {
  ...
  if (flowTaskModalOpenPromise) return flowTaskModalOpenPromise;

  flowTaskModalOpenPromise = new Promise((resolve) => {
    mountFlowTaskModal(resolve);
  }).finally(() => {
    flowTaskModalOpenPromise = null;
  });

  return flowTaskModalOpenPromise;
}

function flowTaskModalStyles() {
  return `
    :host { all: initial; }
    .rmw-overlay {
      position: fixed;
...
function escapeFlowTaskHtml(value) {
...
function createFlowPickerSection(container, { key, title, radioGroupName, fetchItems, metaFor, onSelectionChange }) {
  const section = document.createElement('div');
  section.className = 'rmw-section';
  section.innerHTML = `
    <p class="rmw-section-title">${escapeFlowTaskHtml(title)}</p>
    <input class="rmw-search" type="text" placeholder="Search…" style="display:none" aria-label="Search ${escapeFlowTaskHtml(title)}" />
  ...
  function renderOptions(filterText) {
    ...
    visible.forEach((item) => {
      ...
      meta.innerHTML = parts.map((part) => `<span class="rmw-badge">${escapeFlowTaskHtml(part)}</span>`).join('');
  ...
  async function load() {
    ...
    searchEl.style.display = state.items.length > FLOW_PICKER_SEARCH_THRESHOLD ? 'block' : 'none';
...
function mountFlowTaskModal(resolve) {
  document.getElementById(FLOW_TASK_MODAL_HOST_ID)?.remove();

  const host = document.createElement('div');
  host.id = FLOW_TASK_MODAL_HOST_ID;
  const shadow = host.attachShadow({ mode: 'open' });

  const style = document.createElement('style');
  style.textContent = flowTaskModalStyles();
  ...
  const taskSection = createFlowPickerSection(bodyEl, {
    key: 'task',
    title: 'Select Task',
    radioGroupName: 'rmw-flow-task-choice',
    onSelectionChange: updateContinueState,
    metaFor: (task) => {
      const parts = [flowStatusLabel(task.status)];
    ...
    fetchItems: async () => {
      const result = await fetchMyActiveFlowTasks();
  ...
  const clientSection = createFlowPickerSection(bodyEl, {
    key: 'client',
    title: 'Select Client',
    radioGroupName: 'rmw-flow-client-choice',
    onSelectionChange: updateContinueState,
    metaFor: () => [],
    fetchItems: async () => {
      const result = await fetchActiveFlowClients();
  ...
  // "click outside closes me" listener (its lightbox, in that case), since
  // a Shadow-DOM-hosted event still bubbles out through the shadow boundary
  // to document. Stop every interaction here from propagating past this
  // dialog's own overlay - this modal is a full-screen overlay while open,
  // so the underlying page has no legitimate reason to see any of these
  // events at all. See content-freepik-task-modal.js's identical fix for
