(() => {
const GOOGLE_SCRIPT_VERSION = 'debug-2026-07-21-google-02';
...
function isEnhancorGoogleFlow(toolSlug = STATE.toolSlug) {
  const normalizedToolSlug = normalizeToolSlug(toolSlug || inferToolSlugFromGooglePage());
  return normalizedToolSlug === 'enhancor';
}

function isGensparkGoogleFlow(toolSlug = STATE.toolSlug) {
  const normalizedToolSlug = normalizeToolSlug(toolSlug || inferToolSlugFromGooglePage());
  return normalizedToolSlug === 'genspark';
}

function isElevenLabsGoogleFlow(toolSlug = STATE.toolSlug) {
  const normalizedToolSlug = normalizeToolSlug(toolSlug || inferToolSlugFromGooglePage());
...
function isSunoGoogleFlow(toolSlug = STATE.toolSlug) {
  const normalizedToolSlug = normalizeToolSlug(toolSlug || inferToolSlugFromGooglePage());
  return normalizedToolSlug === 'suno';
}

function isEpidemicSoundGoogleFlow(toolSlug = STATE.toolSlug) {
  const normalizedToolSlug = normalizeToolSlug(toolSlug || inferToolSlugFromGooglePage());
  return normalizedToolSlug === 'epidemic-sound';
}

function isSpliceGoogleFlow(toolSlug = STATE.toolSlug) {
  const normalizedToolSlug = normalizeToolSlug(toolSlug || inferToolSlugFromGooglePage());
  return normalizedToolSlug === 'splice';
}
...
function hasFreepikGooglePasswordValue(input, expectedValue, toolSlug = STATE.toolSlug) {
  ...
  return `${input?.value || ''}` === passwordValue
    || getGooglePasswordTypedMarker(input) === passwordValue;
}
...
function ensureStatusBadge() {
  ...
  badge.textContent = `Google auto-login ${GOOGLE_SCRIPT_VERSION}\n${STATE.status || 'Starting auto-login'}`;
...
function inferToolSlugFromGooglePage() {
  try {
    ...
    if (values.some((value) => (
      value.includes('suno.com')
      || value.includes('studio-api.prod.suno.com')
    ))) {
      return 'suno';
    }

    if (values.some((value) => (
      value.includes('epidemicsound.com')
      || value.includes('login.epidemicsound.com')
    ))) {
      return 'epidemic-sound';
    }

    if (values.some((value) => (
      value.includes('splice.com')
      || value.includes('auth.splice.com')
    ))) {
      return 'splice';
    }
  } catch {}
  ...
  if (
    currentPageText.includes('continue to elevenlabs')
    || currentPageText.includes('continue to eleven labs')
    || currentPageText.includes('to continue to elevenlabs')
    || currentPageText.includes('to continue to eleven labs')
    || currentPageText.includes('review elevenlabs')
    || currentPageText.includes('signing back in to elevenlabs')
    || currentPageText.includes('elevenlabs privacy policy')
    || currentPageText.includes('elevenlabs terms of service')
  ) {
    return 'elevenlabs';
  }
  ...
  if (
    currentPageText.includes('continue to suno')
    || currentPageText.includes('to continue to suno')
    || currentPageText.includes('review suno')
    || currentPageText.includes('signing back in to suno')
    || currentPageText.includes('suno privacy policy')
    || currentPageText.includes('suno terms of service')
  ) {
    return 'suno';
  }

  if (
    currentPageText.includes('continue to epidemic sound')
    || currentPageText.includes('to continue to epidemic sound')
    || currentPageText.includes('continue to epidemicsound')
    || currentPageText.includes('review epidemic sound')
    || currentPageText.includes('signing back in to epidemic sound')
    || currentPageText.includes('epidemic sound privacy policy')
    || currentPageText.includes('epidemic sound terms of service')
  ) {
    return 'epidemic-sound';
  }

  if (
    currentPageText.includes('continue to splice')
    || currentPageText.includes('to continue to splice')
    || currentPageText.includes('review splice')
    || currentPageText.includes('signing back in to splice')
    || currentPageText.includes('splice privacy policy')
    || currentPageText.includes('splice terms of service')
  ) {
    return 'splice';
  }
...
function clearProtectedGooglePasswordMaskLoop() {
  ...
  STATE.passwordTypeObserver = null;
  STATE.passwordTypeTarget = null;
}
...
function ensureProtectedGooglePasswordTypeObserver(passInput) {
  ...
  if (STATE.passwordTypeObserver && STATE.passwordTypeTarget === passInput) {
    return;
  }
  ...
  STATE.passwordTypeObserver = new MutationObserver(() => {
    const activePasswordInput = STATE.passwordTypeTarget;
    if (!activePasswordInput || !activePasswordInput.isConnected) {
      clearProtectedGooglePasswordMaskLoop();
      return;
    }
    enforceProtectedGooglePasswordMask(activePasswordInput);
  });
  STATE.passwordTypeObserver.observe(passInput, {
    attributes: true,
...
function ensureProtectedGooglePasswordMaskLoop(passInput) {
  if (!shouldProtectGooglePasswordReveal()) return;
  if (!passInput) return;
  ensureProtectedGooglePasswordTypeObserver(passInput);
}
...
function enforceProtectedGooglePasswordMask(passInput) {
  if (!passInput) return;
  if (`${passInput.type || ''}`.trim().toLowerCase() !== 'password') {
...
function isGooglePolicyFooterAction(element) {
  if (!element) return false;
  const label = actionText(element);
...
// Coordinate-based click on the exact rendered text (e.g. "Use another account"
// or the credential email) so Google's obfuscated chooser DOM cannot defeat the
// row finders. Targets the precise text position, so it never hits a wrapper.
function clickGoogleChooserTextNode(textValues = []) {
  const values = textValues.map(normalizeText).filter(Boolean);
  ...
  while (node) {
    ...
    if (text && values.some((value) => text === value || text.includes(value))) {
      ...
      if (parent && isVisible(parent)) {
        try {
          ...
          if (rect && rect.width > 0 && rect.height > 0) {
            ...
            const pointed = document.elementFromPoint(pointX, pointY);
            const target = findRealClickableTarget(pointed) || pointed || parent;
            if (target && isVisible(target) && activateActionElement(target)) {
              return true;
            }
          }
        } catch {}
...
function isGoogleConsentContinueScreen() {
  ...
  const mentionsEnhancor = text.includes("you're signing back in to enhancor")
    ...
    || text.includes('signing back in to enhancor')
    || text.includes('continue to enhancor')
    || text.includes('to continue to enhancor')
    || text.includes('review enhancor')
    || text.includes("enhancor's privacy")
    || text.includes('enhancor privacy policy')
    || text.includes('enhancor terms of service');

  const mentionsGenspark = text.includes("you're signing back in to genspark")
    || text.includes('youre signing back in to genspark')
    || text.includes('signing back in to genspark')
    || text.includes('continue to genspark')
    || text.includes('to continue to genspark')
    || text.includes('review genspark')
    || text.includes("genspark's privacy")
    || text.includes('genspark privacy policy')
    || text.includes('genspark terms of service');

  const mentionsElevenLabs = text.includes("you're signing back in to elevenlabs")
    || text.includes('youre signing back in to elevenlabs')
    || text.includes('signing back in to elevenlabs')
    || text.includes('continue to elevenlabs')
    || text.includes('to continue to elevenlabs')
    || text.includes('review elevenlabs')
    || text.includes("elevenlabs's privacy")
    || text.includes("elevenlabs' privacy")
  ...
  const mentionsSuno = text.includes("you're signing back in to suno")
    || text.includes('youre signing back in to suno')
    || text.includes('signing back in to suno')
    || text.includes('continue to suno')
    || text.includes('to continue to suno')
    || text.includes('review suno')
    || text.includes("suno's privacy")
    || text.includes('suno privacy policy')
    || text.includes('suno terms of service');

  const mentionsEpidemicSound = text.includes("you're signing back in to epidemic sound")
    || text.includes('youre signing back in to epidemic sound')
    || text.includes('signing back in to epidemic sound')
    || text.includes('continue to epidemic sound')
    || text.includes('to continue to epidemic sound')
    || text.includes('review epidemic sound')
    || text.includes("epidemic sound's privacy")
    || text.includes('epidemic sound privacy policy')
    || text.includes('epidemic sound terms of service');

  const mentionsSplice = text.includes("you're signing back in to splice")
    || text.includes('youre signing back in to splice')
    || text.includes('signing back in to splice')
    || text.includes('continue to splice')
    || text.includes('to continue to splice')
    || text.includes('review splice')
    || text.includes("splice's privacy")
...
function findGoogleConsentContinueButton() {
  const matchers = ['continue', 'yes, continue', 'confirm', 'allow'];
  const roots = [
    ...Array.from(document.querySelectorAll('[role="dialog"], [aria-modal="true"], main, section, form')),
    document,
  ];

  for (const root of roots) {
    const actions = Array.from(root.querySelectorAll(ACTION_SELECTORS))
      .filter((element) => isVisible(element) && !isDisabled(element))
      .filter((element) => !isGooglePolicyFooterAction(element));

    const exact = actions.find((element) => {
      const label = actionText(element);
    ...
    const partial = actions.find((element) => {
      const label = actionText(element);
  ...
  const fallback = findVisibleActionByText(matchers);
  if (fallback && !isGooglePolicyFooterAction(fallback) && !actionText(fallback).includes('cancel')) {
    return fallback;
  }
  return null;
}
...
async function submitGooglePasswordStep(credential) {
  ...
  if (!passwordValue) return false;
  const normalizedToolSlug = normalizeToolSlug(STATE.toolSlug || inferToolSlugFromGooglePage());
  const allowWithoutPasswordSavingSuppression = isFreepikGoogleFlow(normalizedToolSlug);
  if (!allowWithoutPasswordSavingSuppression && !STATE.passwordSavingSuppressed && !STATE.passwordSavingBypass) return false;
  ...
  if (!allowWithoutPasswordSavingSuppression) {
...
async function attemptKlingGoogleChooserStep(credential) {
  ...
  // Fall back to "Use another account" whenever the credential account is not in
  // the chooser list — even for tools not in shouldPreferGoogleAddAccount — so we
  // never sit forever on the chooser. (Matches the non-wrapper chooser path.)
  const resolvedAddAccountAction = addAccountAction || findGoogleUseAnotherAccountAction();
  if (resolvedAddAccountAction) {
    setStatus('Choosing Google add account');
    if (!activateActionElement(resolvedAddAccountAction)) {
  ...
  // Last resort: click the rendered "Use another account" text directly.
  if (clickGoogleChooserTextNode(['use another account', 'add another account', 'add account'])) {
    setStatus('Choosing Google add account');
    STATE.googleAddAccountPendingAt = Date.now();
    STATE.emailSubmitted = false;
    scheduleAttempt(700);
...
async function attemptKlingGooglePasswordStep(credential) {
  ...
  if (!passwordValue && supportsPasswordOptionalGoogleCredential()) {
    STATE.settled = true;
    setStatus(`Google password step needs manual completion for this ${getToolDisplayName()} account.`);
    releasePasswordSavingSuppressed(0);
    return true;
  }
  if (!passwordValue) {
    STATE.settled = true;
    setStatus(`Google password missing. Check the ${getToolDisplayName()} credential or continue manually.`);
    releasePasswordSavingSuppressed(0);
    return true;
  }
...
async function attemptEnhancorGooglePopupFlow(credential) {
  if (!isEnhancorGoogleFlow()) return false;
  ...
  if (isKlingGoogleRelevantSurface()) {
    setStatus('Waiting for Enhancor Google popup step');
    scheduleAttempt(300);
    return true;
  }
  return false;
...
async function attemptGensparkGooglePopupFlow(credential) {
  if (!isGensparkGoogleFlow()) return false;
  ...
  if (await attemptKlingGoogleDeveloperInfoStep()) return true;
  // Genspark uses Google's standard chooser/password pages; reuse the stable
  // Kling sequence but keep this route isolated from Kling itself.
  if (!isGooglePasswordUrl() && await attemptKlingGoogleChooserStep(credential)) return true;
  if (await attemptKlingGooglePasswordStep(credential)) return true;
  if (await attemptGoogleConsentContinueStep()) return true;
  ...
  if (isKlingGoogleRelevantSurface()) {
    setStatus('Waiting for Genspark Google popup step');
    scheduleAttempt(300);
    return true;
  }
  return false;
...
async function attemptElevenLabsGooglePopupFlow(credential) {
  if (!isElevenLabsGoogleFlow()) return false;
  ...
  if (await attemptKlingGoogleDeveloperInfoStep()) return true;
  // ElevenLabs uses the same Google account chooser structure as Kling. Avoid
  // treating the password page's "Use another account" link as a chooser.
  if (!isGooglePasswordUrl() && await attemptKlingGoogleChooserStep(credential)) return true;
  if (await attemptKlingGooglePasswordStep(credential)) return true;
  if (await attemptGoogleConsentContinueStep()) return true;
  if (await attemptKlingGoogleEmailStep(credential)) return true;
  ...
  if (isKlingGoogleRelevantSurface()) {
    setStatus('Waiting for ElevenLabs Google popup step');
    scheduleAttempt(300);
    return true;
  }
  return false;
...
async function attemptSunoGooglePopupFlow(credential) {
  if (!isSunoGoogleFlow()) return false;
  ...
  if (await attemptKlingGoogleDeveloperInfoStep()) return true;
  // Suno uses Google's standard chooser/password pages; reuse the stable
  // Kling sequence while keeping this route isolated from Kling itself.
  if (!isGooglePasswordUrl() && await attemptKlingGoogleChooserStep(credential)) return true;
  if (await attemptFlowTotpStep()) return true;
  if (await attemptKlingGooglePasswordStep(credential)) return true;
  ...
  if (isKlingGoogleRelevantSurface()) {
    setStatus('Waiting for Suno Google popup step');
    scheduleAttempt(300);
    return true;
  }
  return false;
...
async function attemptEpidemicSoundGooglePopupFlow(credential) {
  if (!isEpidemicSoundGoogleFlow()) return false;
  ...
  if (await attemptKlingGoogleDeveloperInfoStep()) return true;
  // Epidemic Sound uses Google's standard chooser/password pages; reuse the
  // stable Kling sequence while keeping this route isolated from Kling itself.
  if (!isGooglePasswordUrl() && await attemptKlingGoogleChooserStep(credential)) return true;
  if (await attemptFlowTotpStep()) return true;
  if (await attemptKlingGooglePasswordStep(credential)) return true;
  if (await attemptGoogleConsentContinueStep()) return true;
  ...
  if (isKlingGoogleRelevantSurface()) {
    setStatus('Waiting for Epidemic Sound Google popup step');
    scheduleAttempt(300);
    return true;
  }
  return false;
...
async function attemptSpliceGooglePopupFlow(credential) {
  if (!isSpliceGoogleFlow()) return false;
  ...
  if (await attemptKlingGoogleDeveloperInfoStep()) return true;
  // Splice uses Google's standard chooser/password pages; reuse the stable
  // Kling sequence while keeping this route isolated from Kling itself.
  if (!isGooglePasswordUrl() && await attemptKlingGoogleChooserStep(credential)) return true;
  if (await attemptFlowTotpStep()) return true;
  if (await attemptKlingGooglePasswordStep(credential)) return true;
  ...
  if (isKlingGoogleRelevantSurface()) {
    setStatus('Waiting for Splice Google popup step');
    scheduleAttempt(300);
    return true;
  }
  return false;
...
function isFreepikGoogleEmailScreen() {
  if (isGooglePasswordUrl()) return false;
  if (isGoogleAccountChooserPage() || isGoogleDeveloperInfoDialogVisible()) {
    return false;
  }
  if (isFreepikGooglePasswordScreen()) return false;
...
function isFreepikGooglePasswordScreen() {
  const text = pageText();
  return Boolean(
    isGooglePasswordUrl()
    || document.querySelector('#passwordNext')
...
async function attemptFreepikGoogleDeveloperInfoStep() {
  const dismissAction = findGoogleDeveloperInfoDismissAction();
  if (!dismissAction) return false;
...
async function attemptFreepikGoogleEmailStep(credential) {
  if (isGooglePasswordUrl() || document.querySelector('#passwordNext')) return false;
  if (!isFreepikGoogleEmailScreen()) return false;
...
async function attemptFreepikGooglePopupFlow(credential) {
  if (!isFreepikGoogleFlow()) return false;
...
async function attemptPasswordStep(credential) {
  ...
  const passwordValue = `${credential?.password || ''}`;
  const normalizedToolSlug = normalizeToolSlug(STATE.toolSlug || inferToolSlugFromGooglePage());
  const skipPasswordSavingSuppression = normalizedToolSlug === 'freepik';
  ...
  if (!skipPasswordSavingSuppression && !STATE.passwordSavingSuppressed && !STATE.passwordSavingBypass) {
    if (
      STATE.passwordSavingInFlight
      && STATE.passwordSavingInFlightSince
      && Date.now() - STATE.passwordSavingInFlightSince > 4000
  ...
  if (isGooglePasswordUrl() || document.querySelector('#passwordNext')) {
    STATE.emailSubmitted = false;
    if (skipPasswordSavingSuppression && STATE.passwordSubmitted && `${input.value || ''}` !== passwordValue) {
      if (hasGooglePasswordRejectedError(input) || hasGooglePasswordMissingError(input)) {
        STATE.passwordSubmitted = false;
        STATE.lastPasswordSubmitAt = 0;
        STATE.passwordSubmitReadyAt = 0;
        setGooglePasswordTypedMarker(input, '');
      }
    }
    ...
    if (hasGooglePasswordMissingError(input) && !hasFreepikGooglePasswordValue(input, passwordValue, normalizedToolSlug)) {
      STATE.passwordSubmitted = false;
      STATE.lastPasswordFilledAt = 0;
      STATE.passwordSubmitReadyAt = 0;
      setGooglePasswordTypedMarker(input, '');
...
async function attemptFill() {
  ...
  if (await attemptSunoGooglePopupFlow(credential)) return;
  if (await attemptEpidemicSoundGooglePopupFlow(credential)) return;
...
function handleMutations() {
  ...
  if (passwordInput) {
    ensureProtectedGooglePasswordMaskLoop(passwordInput);
  } else if (STATE.passwordMaskLoopTimer || STATE.passwordTypeObserver) {
    clearProtectedGooglePasswordMaskLoop();
  }
