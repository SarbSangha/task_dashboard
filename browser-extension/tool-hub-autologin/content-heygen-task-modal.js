// content-heygen-task-modal.js — Task Selection Modal.
//
// Renders inside a Shadow DOM root - isolation from HeyGen's own stylesheet
// matters here because this is a real interactive form blocking a real
// action. Mirrors content-freepik-task-modal.js (this file's template)
// near-verbatim, renamed for HeyGen.
//
// Shows two INDEPENDENT pickers - Task (content-heygen-task-api.js's
...
// content-heygen.js's click interceptor is the only caller.

const HEYGEN_TASK_MODAL_HOST_ID = 'rmw-heygen-task-gate-host';
const HEYGEN_PICKER_SEARCH_THRESHOLD = 10;

let heygenTaskModalOpenPromise = null;

function openHeygenTaskSelectionModal() {
  // Guards "double-click Generate/Render Scene while the modal is already
  // open" and "popup opened twice" - both collapse to the one in-flight
  // picker.
  if (heygenTaskModalOpenPromise) return heygenTaskModalOpenPromise;

  heygenTaskModalOpenPromise = new Promise((resolve) => {
    mountHeygenTaskModal(resolve);
  }).finally(() => {
    heygenTaskModalOpenPromise = null;
  });

  return heygenTaskModalOpenPromise;
}

function heygenTaskModalStyles() {
  return `
    :host { all: initial; }
    .rmw-overlay {
      position: fixed;
...
function heygenStatusLabel(status) {
  const map = {
...
function escapeHeygenTaskHtml(value) {
...
function createHeygenPickerSection(container, { key, title, radioGroupName, fetchItems, metaFor, onSelectionChange }) {
  const section = document.createElement('div');
  section.className = 'rmw-section';
  section.innerHTML = `
    <p class="rmw-section-title">${escapeHeygenTaskHtml(title)}</p>
    <input class="rmw-search" type="text" placeholder="Search…" style="display:none" aria-label="Search ${escapeHeygenTaskHtml(title)}" />
  ...
  function renderOptions(filterText) {
    ...
    visible.forEach((item) => {
      ...
      meta.innerHTML = parts.map((part) => `<span class="rmw-badge">${escapeHeygenTaskHtml(part)}</span>`).join('');
  ...
  async function load() {
    ...
    // fetchItems() (heygenTaskApiSendMessage) is meant to never reject -
    // it normalizes every failure into { ok: false, error } - but this
    // try/catch is the backstop against any future/unforeseen throw slipping
    // through: without it, an unhandled rejection here leaves the spinner
    // above stuck on "Loading…" forever with no Retry affordance, since
    // nothing else in this call chain has a .catch either.
    let result;
    try {
    ...
    searchEl.style.display = state.items.length > HEYGEN_PICKER_SEARCH_THRESHOLD ? 'block' : 'none';
...
function mountHeygenTaskModal(resolve) {
  document.getElementById(HEYGEN_TASK_MODAL_HOST_ID)?.remove();

  const host = document.createElement('div');
  host.id = HEYGEN_TASK_MODAL_HOST_ID;
  const shadow = host.attachShadow({ mode: 'open' });

  const style = document.createElement('style');
  style.textContent = heygenTaskModalStyles();
  ...
  const taskSection = createHeygenPickerSection(bodyEl, {
    key: 'task',
    title: 'Select Task',
    radioGroupName: 'rmw-heygen-task-choice',
    onSelectionChange: updateContinueState,
    metaFor: (task) => {
      const parts = [heygenStatusLabel(task.status)];
    ...
    fetchItems: async () => {
      const result = await fetchMyActiveHeygenTasks();
  ...
  const clientSection = createHeygenPickerSection(bodyEl, {
    key: 'client',
    title: 'Select Client',
    radioGroupName: 'rmw-heygen-client-choice',
    onSelectionChange: updateContinueState,
    metaFor: () => [],
    fetchItems: async () => {
      const result = await fetchActiveHeygenClients();
