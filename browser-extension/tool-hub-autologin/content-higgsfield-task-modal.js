// content-higgsfield-task-modal.js — Task Selection Modal.
//
// Renders inside a Shadow DOM root - a deliberate departure from the plain
// <div> status badges elsewhere in this extension: those are non-interactive
// readouts where a little CSS bleed-through from Higgsfield's own stylesheet
// is harmless, but this is a real interactive form blocking a real action,
// so isolation from the host page's CSS matters. Mirrors
// content-freepik-task-modal.js exactly.
//
// Shows two INDEPENDENT pickers - Task (content-higgsfield-task-api.js's
// fetchMyActiveHiggsfieldTasks) and Client (fetchActiveHiggsfieldClients, an
...
//
// Exposes exactly one function, openHiggsfieldTaskSelectionModal(), returning
...
// available. content-higgsfield.js's click interceptor is the only caller.

const HIGGSFIELD_TASK_MODAL_HOST_ID = 'rmw-higgsfield-task-gate-host';
const HIGGSFIELD_PICKER_SEARCH_THRESHOLD = 10;

let higgsfieldTaskModalOpenPromise = null;

function openHiggsfieldTaskSelectionModal() {
  ...
  if (higgsfieldTaskModalOpenPromise) return higgsfieldTaskModalOpenPromise;

  higgsfieldTaskModalOpenPromise = new Promise((resolve) => {
    mountHiggsfieldTaskModal(resolve);
  }).finally(() => {
    higgsfieldTaskModalOpenPromise = null;
  });

  return higgsfieldTaskModalOpenPromise;
}

function higgsfieldTaskModalStyles() {
  return `
    :host { all: initial; }
    .rmw-overlay {
      position: fixed;
...
function escapeHiggsfieldTaskHtml(value) {
...
function createHiggsfieldPickerSection(container, { key, title, radioGroupName, fetchItems, metaFor, onSelectionChange }) {
  const section = document.createElement('div');
  section.className = 'rmw-section';
  section.innerHTML = `
    <p class="rmw-section-title">${escapeHiggsfieldTaskHtml(title)}</p>
    <input class="rmw-search" type="text" placeholder="Search…" style="display:none" aria-label="Search ${escapeHiggsfieldTaskHtml(title)}" />
  ...
  function renderOptions(filterText) {
    ...
    visible.forEach((item) => {
      ...
      meta.innerHTML = parts.map((part) => `<span class="rmw-badge">${escapeHiggsfieldTaskHtml(part)}</span>`).join('');
  ...
  async function load() {
    ...
    searchEl.style.display = state.items.length > HIGGSFIELD_PICKER_SEARCH_THRESHOLD ? 'block' : 'none';
...
function mountHiggsfieldTaskModal(resolve) {
  document.getElementById(HIGGSFIELD_TASK_MODAL_HOST_ID)?.remove();

  const host = document.createElement('div');
  host.id = HIGGSFIELD_TASK_MODAL_HOST_ID;
  const shadow = host.attachShadow({ mode: 'open' });

  const style = document.createElement('style');
  style.textContent = higgsfieldTaskModalStyles();
  ...
  const taskSection = createHiggsfieldPickerSection(bodyEl, {
    key: 'task',
    title: 'Select Task',
    radioGroupName: 'rmw-higgsfield-task-choice',
    onSelectionChange: updateContinueState,
    metaFor: (task) => {
      const parts = [higgsfieldStatusLabel(task.status)];
    ...
    fetchItems: async () => {
      const result = await fetchMyActiveHiggsfieldTasks();
  ...
  const clientSection = createHiggsfieldPickerSection(bodyEl, {
    key: 'client',
    title: 'Select Client',
    radioGroupName: 'rmw-higgsfield-client-choice',
    onSelectionChange: updateContinueState,
    metaFor: () => [],
    fetchItems: async () => {
      const result = await fetchActiveHiggsfieldClients();
