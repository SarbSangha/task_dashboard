// content-kling-task-modal.js — Task Selection Modal.
//
// Kling's equivalent of content-freepik-task-modal.js - identical shape,
// renamed. Renders inside a Shadow DOM root - a deliberate departure from the
// plain status badges elsewhere in content-kling.js: those are non-
// interactive readouts where a little CSS bleed-through from Kling's own
// stylesheet is harmless, but this is a real interactive form blocking a real
// action, so isolation from the host page's CSS matters.
//
// Shows two INDEPENDENT pickers - Task (content-kling-task-api.js's
...
// content-kling.js's click interceptor is the only caller.

const KLING_TASK_MODAL_HOST_ID = 'rmw-kling-task-gate-host';
const KLING_PICKER_SEARCH_THRESHOLD = 10;

let klingTaskModalOpenPromise = null;

function openKlingTaskSelectionModal() {
  ...
  if (klingTaskModalOpenPromise) return klingTaskModalOpenPromise;

  klingTaskModalOpenPromise = new Promise((resolve) => {
    mountKlingTaskModal(resolve);
  }).finally(() => {
    klingTaskModalOpenPromise = null;
  });

  return klingTaskModalOpenPromise;
}

function klingTaskModalStyles() {
  return `
    :host { all: initial; }
    .rmw-overlay {
      position: fixed;
...
function klingStatusLabel(status) {
  const map = {
...
function escapeKlingTaskHtml(value) {
...
function createKlingPickerSection(container, { key, title, radioGroupName, fetchItems, metaFor, onSelectionChange }) {
  const section = document.createElement('div');
  section.className = 'rmw-section';
  section.innerHTML = `
    <p class="rmw-section-title">${escapeKlingTaskHtml(title)}</p>
    <input class="rmw-search" type="text" placeholder="Search…" style="display:none" aria-label="Search ${escapeKlingTaskHtml(title)}" />
  ...
  function renderOptions(filterText) {
    ...
    visible.forEach((item) => {
      ...
      meta.innerHTML = parts.map((part) => `<span class="rmw-badge">${escapeKlingTaskHtml(part)}</span>`).join('');
  ...
  async function load() {
    ...
    searchEl.style.display = state.items.length > KLING_PICKER_SEARCH_THRESHOLD ? 'block' : 'none';
...
function mountKlingTaskModal(resolve) {
  document.getElementById(KLING_TASK_MODAL_HOST_ID)?.remove();

  const host = document.createElement('div');
  host.id = KLING_TASK_MODAL_HOST_ID;
  const shadow = host.attachShadow({ mode: 'open' });

  const style = document.createElement('style');
  style.textContent = klingTaskModalStyles();
  ...
  const taskSection = createKlingPickerSection(bodyEl, {
    key: 'task',
    title: 'Select Task',
    radioGroupName: 'rmw-kling-task-choice',
    onSelectionChange: updateContinueState,
    metaFor: (task) => {
      const parts = [klingStatusLabel(task.status)];
    ...
    fetchItems: async () => {
      const result = await fetchMyActiveKlingTasks();
  ...
  const clientSection = createKlingPickerSection(bodyEl, {
    key: 'client',
    title: 'Select Client',
    radioGroupName: 'rmw-kling-client-choice',
    onSelectionChange: updateContinueState,
    metaFor: () => [],
    fetchItems: async () => {
      const result = await fetchActiveKlingClients();
