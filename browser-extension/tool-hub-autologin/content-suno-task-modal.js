// content-suno-task-modal.js — Task Selection Modal.
//
// Direct copy of content-elevenlabs-task-modal.js (this file's template),
// renamed ElevenLabs->Suno throughout. See that file's own header comment
// (and content-flow-task-modal.js, the original template) for the full
// design rationale (Shadow DOM isolation, two independent pickers, Task
// optional / Client required) - reproduced here since this file is the one
// actually loaded for Suno.
//
...
//
// Shows two INDEPENDENT pickers - Task (content-suno-task-api.js's
// fetchMyActiveSunoTasks) and Client (fetchActiveSunoClients, an
...
//
// Exposes exactly one function, openSunoTaskSelectionModal(), returning a
...
// available. content-suno-capture.js's click interceptor is the only caller.

const SUNO_TASK_MODAL_HOST_ID = 'rmw-suno-task-gate-host';
const SUNO_PICKER_SEARCH_THRESHOLD = 10;

let sunoTaskModalOpenPromise = null;

function openSunoTaskSelectionModal() {
  // Guards "double-click Create while the modal is already open" and "popup
  // opened twice" - both collapse to the one in-flight picker.
  if (sunoTaskModalOpenPromise) return sunoTaskModalOpenPromise;

  sunoTaskModalOpenPromise = new Promise((resolve) => {
    mountSunoTaskModal(resolve);
  }).finally(() => {
    sunoTaskModalOpenPromise = null;
  });

  return sunoTaskModalOpenPromise;
}

function sunoTaskModalStyles() {
  return `
    :host { all: initial; }
    .rmw-overlay {
      position: fixed;
...
function sunoStatusLabel(status) {
  const map = {
...
function escapeSunoTaskHtml(value) {
...
function createSunoPickerSection(container, { key, title, radioGroupName, fetchItems, metaFor, onSelectionChange }) {
  const section = document.createElement('div');
  section.className = 'rmw-section';
  section.innerHTML = `
    <p class="rmw-section-title">${escapeSunoTaskHtml(title)}</p>
    <input class="rmw-search" type="text" placeholder="Search…" style="display:none" aria-label="Search ${escapeSunoTaskHtml(title)}" />
  ...
  function renderOptions(filterText) {
    ...
    visible.forEach((item) => {
      ...
      meta.innerHTML = parts.map((part) => `<span class="rmw-badge">${escapeSunoTaskHtml(part)}</span>`).join('');
  ...
  async function load() {
    ...
    searchEl.style.display = state.items.length > SUNO_PICKER_SEARCH_THRESHOLD ? 'block' : 'none';
...
function mountSunoTaskModal(resolve) {
  document.getElementById(SUNO_TASK_MODAL_HOST_ID)?.remove();

  const host = document.createElement('div');
  host.id = SUNO_TASK_MODAL_HOST_ID;
  const shadow = host.attachShadow({ mode: 'open' });

  const style = document.createElement('style');
  style.textContent = sunoTaskModalStyles();
  ...
  const taskSection = createSunoPickerSection(bodyEl, {
    key: 'task',
    title: 'Select Task',
    radioGroupName: 'rmw-suno-task-choice',
    onSelectionChange: updateContinueState,
    metaFor: (task) => {
      const parts = [sunoStatusLabel(task.status)];
    ...
    fetchItems: async () => {
      const result = await fetchMyActiveSunoTasks();
  ...
  const clientSection = createSunoPickerSection(bodyEl, {
    key: 'client',
    title: 'Select Client',
    radioGroupName: 'rmw-suno-client-choice',
    onSelectionChange: updateContinueState,
    metaFor: () => [],
    fetchItems: async () => {
      const result = await fetchActiveSunoClients();
