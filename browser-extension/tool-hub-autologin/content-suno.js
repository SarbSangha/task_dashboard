const TOOL_SLUG = 'suno';
const LOGIN_URL = 'https://suno.com/';
...
const STATE = {
  ...
  status: 'Waiting for Suno login form',
};
...
// Suno is a Next.js SSR app: acting before React finishes hydrating trips a
...
function ensureStatusBadge() {
  const existing = document.getElementById('rmw-suno-autologin-status');
  if (existing) return existing;

  const badge = document.createElement('div');
  badge.id = 'rmw-suno-autologin-status';
  ...
  badge.textContent = `Suno auto-login ${SCRIPT_VERSION}\n${STATE.status || 'Starting auto-login'}`;
...
function setStatus(message) {
  ...
  console.debug('[RMW Suno Auto Login]', message);
}

function debugLog(label, data = {}) {
  console.log(`[RMW Suno Auto Login] ${label}`, data);
}

function exposeDebugState() {
  try {
    window.__RMW_STATE = STATE;
    window.__RMW_SUNO_DEBUG = {
      state: STATE,
      attemptFill,
      attemptOpenSunoLogin,
...
function diagnoseActionStep({ label, textValues, finder }) {
  ...
  console.log(`[RMW Suno Auto Login] ${label} diagnostic result`, result);
  return result;
}
...
function clickAction(element, label, delays = [0, 250, 900]) {
  if (!element) return false;
  console.log('[RMW Suno Auto Login] clicking action', label, describeElement(element));
  delays.forEach((delay) => {
    window.setTimeout(() => {
      if (!isVisible(element) || isDisabled(element)) return;
      console.log('[RMW Suno Auto Login] click attempt', label, delay);
      safeClick(element);
...
// Fresh (incognito) sessions always show Suno's cookie-consent banner, whose
...
function maybeDismissCookieConsent() {
  ...
  if (consentAction) {
    ...
    debugLog('Dismissing Suno cookie consent', describeElement(consentAction));
...
function onSunoHost() {
  return window.location.hostname === 'suno.com'
    || window.location.hostname.endsWith('.suno.com');
}

function isLandingPage() {
  const host = window.location.hostname;
  const path = `${window.location.pathname || '/'}`;
  return (host === 'suno.com' || host === 'www.suno.com') && (path === '/' || path === '');
}

function isLoginPage() {
  return Boolean(findEmailInput())
    || Boolean(findPasswordInput())
    || Boolean(findEmailOptionAction())
    || Boolean(findGoogleOptionAction())
    || Boolean(findUsePasswordInsteadAction())
    || (onSunoHost() && Boolean(findHomeSignInAction()));
}

function looksLikeAuthenticatedWorkspace() {
  if (!onSunoHost()) return false;
  ...
  const workspaceWords = ['create', 'library', 'explore', 'workspace', 'home', 'personas', 'credits', 'upgrade', 'search'];
    ...
    (response) => {
      ...
      window.setTimeout(() => {
        ...
        try {
          ...
          setStatus(`Suno direct run error: ${error?.message || 'Unknown error'}`);
          releasePasswordSavingSuppressed(0);
        }
      }, 0);
...
function schedulePasswordSwitchAfterEmailChoice(attempt = 1) {
  ...
  STATE.passwordSwitchTimer = window.setTimeout(() => {
    ...
    if (passwordOption || emailInput) {
      setStatus(`Opening Suno password sign-in (attempt ${attempt})`);
    ...
    if (attempt < 8) {
      ...
      setStatus('Suno password option not found after email choice');
      forceScheduleAttempt(200);
    }
  }, attempt === 1 ? 500 : 750);
}

function attemptOpenSunoLogin() {
  debugLog('attemptOpenSunoLogin CALLED', {
    canActNow: canActNow(),
  ...
  const emailInput = findEmailInput();
  const passwordInput = findPasswordInput();
  debugLog('attemptOpenSunoLogin fields', {
    emailInput: describeElement(emailInput),
    passwordInput: describeElement(passwordInput),
  });
  if (emailInput || passwordInput) {
    debugLog('attemptOpenSunoLogin EXIT existing fields');
    return false;
  }
  ...
  if (shouldUseGoogleProvider()) {
    ...
    if (googleOption) {
      if (canActNow()) {
        ...
        setStatus('Opening Suno Google sign-in');
  ...
  if (emailOption) {
    if (canActNow()) {
      markActionTaken();
      STATE.loginOpenAttempts += 1;
      setStatus('Opening Suno email sign-in');
  ...
  if (homeSignIn) {
    if (canActNow()) {
      markActionTaken();
      STATE.loginOpenAttempts += 1;
      setStatus('Opening Suno sign-in');
  ...
  if (!onSunoHost() && canActNow()) {
    markActionTaken();
    STATE.loginOpenAttempts += 1;
    setStatus('Redirecting to Suno sign-in');
    window.location.replace(LOGIN_URL);
    return true;
  }
...
function attemptProviderChoice(source = 'provider-choice') {
  ...
  setStatus(`Inspecting Suno ${route} sign-in (${source})\n${stateSnapshotText()}`);
  ...
  if ((emailInput || passwordInput) && !emailOption && !googleOption) {
    setStatus(`Suno form already open (${source})\n${stateSnapshotText()}`);
    return false;
  }

  setStatus(`Choosing Suno email sign-in (${source})`);
    if (emailOption) {
      clickAction(emailOption, 'Use email');
  ...
  setStatus('Suno email sign-in option not found');
  return false;
}
...
function attemptSwitchToPassword(emailInput) {
  ...
  if (!usePasswordAction) {
    if (canActNow() && clickVisibleText(['use password', 'use password instead', 'sign in with password', 'log in with password'], 'Use password')) {
      markActionTaken();
      setStatus('Switching Suno to password login');
  ...
  if (!STATE.credential?.loginIdentifier) {
    requestCredential();
    setStatus('Waiting for Suno email credential');
    return true;
  }
...
function stopAutomation(message, hideBadgeAfterMs = 2500) {
  ...
  if (hideBadgeAfterMs > 0) {
    window.setTimeout(() => {
      const badge = document.getElementById('rmw-suno-autologin-status');
      if (badge) {
        badge.remove();
      }
    }, hideBadgeAfterMs);
...
function attemptFill() {
  ...
  if (!STATE.credential?.loginIdentifier || (!STATE.credential?.password && !shouldUseGoogleProvider())) {
    ...
    if (emailInput || passwordInput || emailOption || passwordOption || findGoogleOptionAction() || findHomeSignInAction()) {
      requestCredential();
    }
    if (attemptOpenSunoLogin()) {
      return;
    }
  ...
  if (shouldUseGoogleProvider()) {
    if (attemptProviderChoice('attempt-fill') || attemptOpenSunoLogin()) {
      return;
    }
    setStatus('Waiting for Suno Google sign-in option');
    return;
  }
  ...
  if (emailInput && !passwordInput) {
    requestPasswordSavingSuppression();
    setStatus('Waiting for Suno password login');
    return;
  }

  if (!emailInput && !passwordInput) {
    attemptOpenSunoLogin();
    setStatus('Waiting for Suno login field');
    return;
  }
...
function scheduleAttempt(delay = 0) {
  if (STATE.settled) {
    ...
    setStatus(`Suno scheduler skipped: settled\n${stateSnapshotText()}`);
    return;
  }
  if (STATE.scheduledTimer) {
...
function forceScheduleAttempt(delay = 0) {
  if (STATE.settled) {
    ...
    setStatus(`Suno force scheduler skipped: settled\n${stateSnapshotText()}`);
    return;
  }
  if (STATE.scheduledTimer) {
    window.clearTimeout(STATE.scheduledTimer);
