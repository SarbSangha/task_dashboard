...
export default function Tools({ view = 'tools', searchQuery: externalSearchQuery, onSearchChange: externalOnSearchChange }) {
  ...
  // toolSupportsPasswordOptionalCredential is only ever true for Claude, and only
  // meant its classic passwordless email-magic-link flow (no password concept at
  // all there) - the field should stay hidden for that. Once Claude's login
  // method is switched to "google" a real Google password step can appear (e.g.
  // signing in with an account that isn't already authenticated in this browser
  // profile), so the field must render there the same as it does for every other
  // Google-capable tool - hiding it silently strips whatever the admin typed at
  // save time below (see handleSaveCredential's identical guard).
  ...
  const handleSaveCredential = async (event) => {
    ...
    // See the matching activeCredentialShouldHidePasswordField comment above -
    // must stay in sync with it, since this is the flag that actually decides
    // whether credentialForm.password is kept or discarded below.
