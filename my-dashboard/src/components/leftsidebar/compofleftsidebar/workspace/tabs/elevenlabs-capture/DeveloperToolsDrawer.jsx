...
import { formatRelativeTime, normalizeApiError, truncate } from './elevenlabsCaptureUtils';

// Mirrors flow-capture/DeveloperToolsDrawer.jsx, minus DiagnosticsPanel -
// ElevenLabs has no ingest-stats endpoint (no GET
// /api/providers/elevenlabs/metrics) - see providers/elevenlabs/router.py's
// own docstring.
...
function RawEventInspector() {
  ...
  const handleSearch = async (submitEvent) => {
    ...
    setSearched(true);
    try {
      const response = await elevenlabsCaptureAPI.listEvents({ client_event_id: trimmed, limit: 1 });
  ...
  return (
    <div className="chatgpt-capture-panel chatgpt-capture-raw-inspector">
      <div className="chatgpt-capture-panel-head">
        ...
        <p>Look up any raw ElevenlabsCaptureEvent by its client event id (e.g. <code>elevenlabs:0a53c5d4-...:2026-08-11T...</code>) -
...
function LiveCaptureFeedRow({ event }) {
  // ElevenLabs' events are always eventType 'history_row' (unlike Flow,
  // which has two event types - generation_workflow_row/media_url_resolved
  // - and special-cases the latter in its own version of this row). No
  // isMediaUrl branch needed here. Prompt-like text is extracted
  // defensively from the payload trying multiple candidate keys, same
  // priority order as the backend's own normalization.py::_extract_fields.
...
function LiveCaptureFeed({ active }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const response = await elevenlabsCaptureAPI.listEvents({ limit: 10 });
        if (cancelled) return;
