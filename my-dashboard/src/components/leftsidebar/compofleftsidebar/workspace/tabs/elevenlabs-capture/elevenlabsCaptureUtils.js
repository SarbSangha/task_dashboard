// Shared formatting/labeling helpers for the ElevenLabs Capture Center.
// Copied from flow-capture/flowCaptureUtils.js - mirrors
// envato-capture/envatoCaptureUtils.js's "no component imports,
// dependency-free" rule, each provider folder stays self-contained. Same
// "no confirmed item-type field to key off yet" posture as Flow's version -
// ElevenLabs' own `source` field (TTS/Music/SFX/...) is surfaced directly as
// a MetaField in GenerationDetailPanel.jsx instead of being mapped through a
// helper here.
...
export function normalizeApiError(error, fallback) {
  ...
  if (error?.response?.status === 403) {
    return 'Administrator access is required for the ElevenLabs Capture Center.';
  }
