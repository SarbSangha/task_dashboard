// Shared formatting/labeling helpers for the Flow Capture Center. Mirrors
// envato-capture/envatoCaptureUtils.js's "no component imports,
// dependency-free" rule - each provider folder stays self-contained. Drops
// the item-type/audio-video helpers Envato's version has - Flow has no
// confirmed type field to key off yet (see providers/flow/CAPTURE_CONTRACT.md's
// known gaps: video generation's shape has never been captured live).
...
export function normalizeApiError(error, fallback) {
  ...
  if (error?.response?.status === 403) {
    return 'Administrator access is required for the Flow Capture Center.';
  }
