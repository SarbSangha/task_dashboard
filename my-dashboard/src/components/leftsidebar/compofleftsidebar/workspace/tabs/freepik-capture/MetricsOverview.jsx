...
import { formatCount, formatCredits } from './freepikCaptureUtils';
...
export default function MetricsOverview({ metrics, loading, error }) {
  ...
  return (
    <div className="chatgpt-capture-metrics-grid">
      <MetricCard icon="🖼️" tone="primary" label="Generations Captured" value={formatCount(metrics.totalGenerations)} />
      ...
      />
      <MetricCard icon="💳" tone="info" label="Credits Charged (total)" value={formatCredits(metrics.creditsChargedTotal)} />
    </div>
  );
}
