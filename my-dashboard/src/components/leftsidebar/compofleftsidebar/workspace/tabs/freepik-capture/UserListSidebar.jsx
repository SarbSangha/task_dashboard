...
import { formatCount, formatCredits, formatRelativeTime, normalizeApiError } from './freepikCaptureUtils';
...
export default function UserListSidebar({ selectedUserId, onSelectUser }) {
  ...
  const loadUsers = useCallback(
    async (offset, { append } = {}) => {
      ...
      setUsersError('');
      try {
        const response = await freepikCaptureAPI.listUsers({
    ...
    <div className="chatgpt-capture-sidebar-panel">
      ...
      {!usersLoading && !usersError && users.length === 0 && (
        <div className="chatgpt-capture-empty-state compact">
          ...
          <p>Once an employee generates something in Freepik through the dashboard launcher, they'll show up here.</p>
        </div>
      )}
