// Shared formatting/labeling helpers for the Freepik Capture Center. Kept
// dependency-free, mirroring chatgpt-capture/chatgptCaptureUtils.js's own
...
// ../chatgpt-capture/ - each provider folder stays self-contained, same
// precedent as ChatGPT's own folder not reaching into Kling's code.

// Video-generator creations point downloadUrl/rawUrl at an .mp4 (see
// normalization.py's _extract_video_asset) instead of an image - anything
// that would otherwise blindly render a generation's asset URL in an <img>
// needs this check first, or a video creation renders a broken image icon.
...
export function normalizeApiError(error, fallback) {
  ...
  if (error?.response?.status === 403) {
    return 'Administrator access is required for the Freepik Capture Center.';
  }
