...
import { formatCount, formatCredits, formatRelativeTime, normalizeApiError, truncate } from './heygenCaptureUtils';
...
function RawEventInspector() {
  ...
  const handleSearch = async (submitEvent) => {
    ...
    setSearched(true);
    try {
      const response = await heygenCaptureAPI.listEvents({ client_event_id: trimmed, limit: 1 });
  ...
  return (
    <div className="chatgpt-capture-panel chatgpt-capture-raw-inspector">
      <div className="chatgpt-capture-panel-head">
        ...
        <p>Look up any raw HeygenCaptureEvent by its client event id (e.g. <code>heygen:v-12345:submitted</code>) - the
      ...
      {event && (
        <>
          <div className="chatgpt-capture-fields-grid">
            ...
            <div className="chatgpt-capture-field">
              <span className="chatgpt-capture-field-label">Video ID</span>
              <span className="chatgpt-capture-mono">{event.providerVideoId || '—'}</span>
            </div>
          </div>
...
// Derives a human stage label from what a raw HeygenCaptureEvent alone can
// tell us - the backend pipeline (ingest + normalize) completes synchronously
// within one request, so these are sequence-of-completion labels for admin
// legibility, not fabricated delays. Provider-agnostic - identical to
// freepik-capture's version.
...
// Reads content-heygen.js's own capture envelope (payload.scriptText,
// payload.avatar/voice/credits/status) rather than a raw provider API
// response - HeyGen has no confirmed API response shape (see
// providers/heygen/normalization.py's module docstring), so the extension
// builds this envelope itself from DOM scraping + shape-based network
// interception.
function LiveCaptureFeedRow({ event, ownerName }) {
  ...
  return (
    <div className="chatgpt-capture-panel" style={{ marginBottom: 8 }}>
      <div className="chatgpt-capture-panel-head">
        <h4 style={{ fontSize: 13 }}>{truncate(payload.scriptText, 90) || 'No script captured'}</h4>
      ...
      <div className="chatgpt-capture-fields-grid">
        <div className="chatgpt-capture-field">
          <span className="chatgpt-capture-field-label">Avatar</span>
          <span>{avatar.name || '—'}</span>
        </div>
        <div className="chatgpt-capture-field">
          <span className="chatgpt-capture-field-label">Voice</span>
...
function LiveCaptureFeed({ active }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const response = await heygenCaptureAPI.listEvents({ limit: 10 });
        if (cancelled) return;
        ...
        if (unresolvedUserIds.length) {
          const results = await Promise.all(
            unresolvedUserIds.map((userId) => heygenCaptureAPI.getUser(userId).then(
