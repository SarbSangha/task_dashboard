...
import HeygenGenerationGrid from './HeygenGenerationGrid';
import { normalizeApiError } from './heygenCaptureUtils';

const GENERATION_PAGE_SIZE = 24;

/**
 * Fetch/pagination state lives here, presentation lives in the grid - same
 * split freepik-capture/GenerationsBrowser.jsx keeps. Search/ownership/task/
 * client filters are controlled from HeygenExplorerBody (rendered in its
 * header, next to the All Generations/By Employee switcher) rather than
 * owned locally; `total` is reported back up via onTotalChange so that same
 * header can show "N generation(s)".
 */
export default function GenerationsBrowser({
  ...
  const loadGenerations = useCallback(
    async (offset, { append } = {}) => {
      ...
      setError('');
      try {
        const response = await heygenCaptureAPI.listGenerations({
    ...
    <div>
      ...
      {!loading && !error && generations.length === 0 && (
        <div className="chatgpt-capture-empty-state">
          ...
          <p>Open HeyGen through the dashboard launcher with the extension active, or widen your filters.</p>
        </div>
      )}

      <HeygenGenerationGrid
