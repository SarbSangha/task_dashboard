...
import HeygenGenerationDrawer from './HeygenGenerationDrawer';
import DeveloperToolsDrawer from './DeveloperToolsDrawer';
import { normalizeApiError } from './heygenCaptureUtils';
...
// design-system CSS a second time - see freepik-capture/FreepikExplorerBody.jsx
// for the same reasoning.
import '../ChatGptCaptureCenterTab.css';
// Kling's own card/grid CSS (.kling-card*, .kling-virtual-grid-wrap,
// .kling-skeleton-grid) reused the same way, so HeyGen generations look like
// the Kling/Freepik cards.
...
// TrendingsPanel.jsx, same reasoning as freepik-capture's identical import.
...
/**
 * The HeyGen Capture Center UI - structural twin of
 * freepik-capture/FreepikExplorerBody.jsx: a full-width card grid (generations
 * are visual, not text transcripts) with a slide-in drawer for detail, admin-
 * gated the same way (no dedicated permission key - see TrendingsPanel.jsx).
 *
 * Two browse modes: "All Generations" (default - includes unclaimed rows,
 * since not every HeyGen generation has a resolvable owner) and "By Employee".
 */
export default function HeygenExplorerBody({ searchInput = '' }) {
  ...
  // `searchInput` comes from TrendingsPanel's own header, same as Freepik's.
  ...
  const fetchMetrics = useCallback(async ({ silent = false, announce = false } = {}) => {
    ...
    try {
      const response = await heygenCaptureAPI.getMetrics();
      setMetrics(response.data);
      setMetricsError('');
      if (announce) setToast({ type: 'success', message: 'Metrics refreshed.' });
    } catch (error) {
      const message = normalizeApiError(error, 'Unable to load HeyGen Capture Center metrics.');
  ...
  // Task/Client filter dropdown options - every task/client any HeyGen
  // generation has ever been linked to, not just whatever's currently active.
  useEffect(() => {
    if (!isAdmin) return;
    heygenCaptureAPI.getLinkedTasks().then((res) => setLinkedTasks(res.tasks || [])).catch(() => {});
    heygenCaptureAPI.getLinkedClients().then((res) => setLinkedClients(res.clients || [])).catch(() => {});
  }, [isAdmin]);
    ...
    return (
      <div className="tab-content tab-content-projects chatgpt-capture-tab">
        <div className="chatgpt-capture-alert">
          Administrator access is required to use the HeyGen Capture Center.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="tab-content tab-content-projects chatgpt-capture-tab">
      ...
      <HeygenGenerationDrawer generationId={openGenerationId} onClose={() => setOpenGenerationId(null)} />
