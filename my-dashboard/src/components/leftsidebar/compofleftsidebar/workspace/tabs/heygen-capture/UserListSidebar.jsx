...
import { formatCount, formatCredits, formatRelativeTime, normalizeApiError } from './heygenCaptureUtils';
...
export default function UserListSidebar({ selectedUserId, onSelectUser }) {
  ...
  const loadUsers = useCallback(
    async (offset, { append } = {}) => {
      ...
      setUsersError('');
      try {
        const response = await heygenCaptureAPI.listUsers({
    ...
    <div className="chatgpt-capture-sidebar-panel">
      ...
      {!usersLoading && !usersError && users.length === 0 && (
        <div className="chatgpt-capture-empty-state compact">
          ...
          <p>Once an employee generates something in HeyGen through the dashboard launcher, they'll show up here.</p>
        </div>
      )}
