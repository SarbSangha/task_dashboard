// Shared formatting/labeling helpers for the HeyGen Capture Center. Kept
// dependency-free, mirroring freepik-capture/freepikCaptureUtils.js's own
...
// ../freepik-capture/ or ../chatgpt-capture/ - each provider folder stays
// self-contained, same precedent those folders already follow.
//
// No isVideoAssetUrl helper here (unlike Freepik's) - every HeyGen
// generation is a video by construction (it's an AI-avatar video tool), so
// there's no image/video branching to do anywhere in this folder.
...
// Matches HeygenGeneration.status values (backend/providers/heygen/constants.py) -
// the fuller lifecycle the capture spec calls for (pending -> queued ->
// processing/rendering -> completed, or -> failed/cancelled), a superset of
// Freepik's 4-state one.
...
export function normalizeApiError(error, fallback) {
  ...
  if (error?.response?.status === 403) {
    return 'Administrator access is required for the HeyGen Capture Center.';
  }
