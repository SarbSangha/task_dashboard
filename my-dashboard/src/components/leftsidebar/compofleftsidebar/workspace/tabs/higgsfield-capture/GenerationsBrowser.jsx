...
import HiggsfieldGenerationGrid from './HiggsfieldGenerationGrid';
 ...
 * split heygen-capture/GenerationsBrowser.jsx keeps. Search/ownership/task/
 * client filters are controlled from HiggsfieldExplorerBody (rendered in its
...
export default function GenerationsBrowser({
  ...
  const loadGenerations = useCallback(
    async (offset, { append } = {}) => {
      ...
      setError('');
      try {
        const response = await higgsfieldCaptureAPI.listGenerations({
    ...
    <div>
      ...
      {!loading && !error && generations.length === 0 && (
        <div className="chatgpt-capture-empty-state">
          ...
          <p>Open Higgsfield through the dashboard launcher with the extension active, or widen your filters.</p>
        </div>
      )}

      <HiggsfieldGenerationGrid
