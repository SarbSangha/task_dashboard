...
import { formatCount, formatCredits, formatRelativeTime, normalizeApiError } from './higgsfieldCaptureUtils';
    ...
    >
      ...
      <div className="chatgpt-capture-user-card-body">
        ...
        <div className="chatgpt-capture-user-card-meta">
          ...
          <span>💳 {formatCredits(user.creditsUsedTotal)}</span>
...
export default function UserListSidebar({ selectedUserId, onSelectUser }) {
  ...
  const loadUsers = useCallback(
    async (offset, { append } = {}) => {
      ...
      setUsersError('');
      try {
        const response = await higgsfieldCaptureAPI.listUsers({
    ...
    <div className="chatgpt-capture-sidebar-panel">
      ...
      {!usersLoading && !usersError && users.length === 0 && (
        <div className="chatgpt-capture-empty-state compact">
          ...
          <p>Once an employee generates something in Higgsfield through the dashboard launcher, they'll show up here.</p>
        </div>
      )}
