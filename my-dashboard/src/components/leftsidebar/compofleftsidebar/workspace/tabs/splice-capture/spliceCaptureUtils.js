// Shared formatting/labeling helpers for the Splice Capture Center. Mirrors
// epidemicsound-capture/epidemicCaptureUtils.js's own "no component imports,
// dependency-free" rule - each provider folder stays self-contained, so this
// is a fresh copy rather than a cross-folder import (no established
// precedent in this codebase for sharing util modules between provider tab
// folders - every provider folder keeps its own copy of these helpers).
// Splice (splice.com) is a sample/loop library - every download is an audio
// sample (.wav), there is no Sound-Effect-vs-Music split like Epidemic
// Sound's isSfx, no adaptations/generations concept, and no download-quota
// field, so this file skips getSoundKindMeta and any quota formatter
// entirely.
...
export function normalizeApiError(error, fallback) {
  ...
  if (error?.response?.status === 403) {
    return 'Administrator access is required for the Splice Capture Center.';
  }
