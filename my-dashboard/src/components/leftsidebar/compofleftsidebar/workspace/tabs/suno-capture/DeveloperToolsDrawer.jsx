...
// Suno-specific logic inside it) - reused directly from elevenlabs-capture
// instead of duplicating a provider-agnostic file, per that folder's own
// JsonViewer.jsx comment.
import JsonViewer from '../elevenlabs-capture/JsonViewer';
import { formatRelativeTime, normalizeApiError, truncate } from './sunoCaptureUtils';

// Mirrors elevenlabs-capture/DeveloperToolsDrawer.jsx, minus
// DiagnosticsPanel - Suno has no ingest-stats endpoint either (no GET
// /api/providers/suno/metrics), same shape as ElevenLabs' own backend.
...
function RawEventInspector() {
  ...
  const handleSearch = async (submitEvent) => {
    ...
    setSearched(true);
    try {
      const response = await sunoCaptureAPI.listEvents({ client_event_id: trimmed, limit: 1 });
  ...
  return (
    <div className="chatgpt-capture-panel chatgpt-capture-raw-inspector">
      <div className="chatgpt-capture-panel-head">
        ...
        <p>Look up any raw SunoCaptureEvent by its client event id (e.g. <code>suno:0a53c5d4-...:2026-08-17T...</code>) -
...
function LiveCaptureFeedRow({ event }) {
  // Like ElevenLabs, Suno's raw events don't need extra event-type
  // branching here - no multi-event-type special-casing like Flow's
  // generation_workflow_row/media_url_resolved split. Prompt-like text is
  // extracted defensively from the payload trying multiple candidate keys,
  // same priority order as the backend's own normalization.py::_extract_fields.
  const prompt = event.payload?.text || event.payload?.prompt || event.payload?.text_input;
...
function LiveCaptureFeed({ active }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const response = await sunoCaptureAPI.listEvents({ limit: 10 });
        if (cancelled) return;
