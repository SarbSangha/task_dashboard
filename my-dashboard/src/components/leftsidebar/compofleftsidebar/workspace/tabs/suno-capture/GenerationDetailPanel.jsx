...
import { sunoCaptureAPI } from '../../../../../../services/api';
import { UserAvatar } from '../../../../../common/UserAvatar';
// JsonViewer is byte-for-byte identical across every provider folder (no
// Suno-specific logic inside it, just a generic JSON pretty-printer built on
// copyTextToClipboard) - reused directly from elevenlabs-capture instead of
// duplicating a provider-agnostic file, per that folder's own JsonViewer.jsx
// comment.
import JsonViewer from '../elevenlabs-capture/JsonViewer';
import {
  copyTextToClipboard,
  formatAbsoluteTime,
  formatCount,
  ...
  normalizeApiError,
} from './sunoCaptureUtils';
// Renders with Kling's drawer classes - mirrors
// elevenlabs-capture/GenerationDetailPanel.jsx, minus a dedicated owner-user
// fetch/panel (GET /api/providers/suno/users/{id} doesn't exist for this
// provider either) - instead the /generations endpoints batch-attach
// ownerName server-side, same as ElevenLabs, falling back to "User #<id>"
// only if that lookup came back empty (deleted/missing user).
...
export default function GenerationDetailPanel({ generationId }) {
  ...
  const load = useCallback(async (id) => {
    ...
    setError('');
    try {
      const response = await sunoCaptureAPI.getGeneration(id);
  ...
  // mirroredAssetUrl is our own permanent R2 copy of the audio - preferred
  // once it exists, since Suno's own mediaUrl may be time-limited/expiring.
  // Falls back to mediaUrl for anything not mirrored yet - same "our
  // mirrored copy first, provider's original as fallback" idiom
  // elevenlabs-capture/GenerationDetailPanel.jsx uses for mirroredAssetUrl
  // || mediaUrl.
  ...
  return (
    <>
        ...
        {primaryAudioUrl ? (
          ...
          <div className="kling-drawer-preview-empty">
            {generation.assetMirrorStatus === 'skipped'
              ? 'This generation has no audio output (generation failed or was never rendered).'
              : "Audio not available yet — Suno's media URL for this generation hasn't resolved or been mirrored yet."}
          </div>
        )}
      </div>
      ...
      <div className="kling-drawer-section kling-drawer-metadata-grid">
        ...
        <MetaField label="Model" value={generation.modelName} />
        <MetaField label="Model Version" value={generation.majorModelVersion} />
        <MetaField
          label="Credits Used"
