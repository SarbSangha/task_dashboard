...
import SunoGenerationGrid from './SunoGenerationGrid';
import { normalizeApiError } from './sunoCaptureUtils';

const GENERATION_PAGE_SIZE = 24;

// Mirrors elevenlabs-capture/GenerationsBrowser.jsx's fetch/paginate/debounce
// pattern exactly. Same real difference from Flow: GET
// /api/providers/suno/generations does NOT accept a `q` free-text search
// param (only owner_user_id/linked_task_id/linked_client_id/limit/offset -
// same shape as ElevenLabs' equivalent endpoint), so `searchInput` is never
// sent to the server here. Instead it's applied as a client-side substring
// filter over whatever page of generations is already loaded (prompt match,
// case-insensitive) - cheap, no extra endpoint needed, and keeps the header
// search box from being dead weight. The trade-off: `total`/"Load more"
// still reflect the *server's* unfiltered count, so a narrow search can show
// fewer visible cards than the "N generation(s)" header while more matches
// could exist further down un-loaded pages - an acceptable v1 limitation
// given the backend has no server-side search for this provider.
export default function GenerationsBrowser({
  ...
  const loadGenerations = useCallback(
    async (offset, { append } = {}) => {
      ...
      setError('');
      try {
        const response = await sunoCaptureAPI.listGenerations({
    ...
    <div>
      ...
      {!loading && !error && generations.length === 0 && downloadedOnly && (
        <div className="chatgpt-capture-empty-state">
          ...
          <p>Generations show up here once their Download button is actually clicked in Suno.</p>
        </div>
      )}
      ...
      {!loading && !error && generations.length === 0 && !downloadedOnly && (
        <div className="chatgpt-capture-empty-state">
          ...
          <p>Open Suno through the dashboard launcher with the extension active, or widen your filters.</p>
        </div>
      )}
